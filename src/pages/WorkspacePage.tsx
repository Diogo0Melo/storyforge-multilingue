...
interface MasterCopilotGuardState {
  active: boolean
  reason: MasterCopilotGuardReason | null
}

const EMPTY_CANDIDATE_UPDATE_STATE: Record<number, 'updating' | 'settled' | 'failed'> = {}

/**
 * Keep WorkspacePage's outer controls in step with the Master Agent state.
 * ChatCopilotPanel keeps its own controller, so the staged hook's scope sync
 * is the source for long-running work while the panel remains authoritative
 * for the candidate update/quarantine details visible to the author.
 */
function WorkspaceCopilotGuard({
  project,
  worldGroupId,
  onChange,
}: {
  ...
  onChange: (state: MasterCopilotGuardState) => void
}) {
  const copilot = useMasterCopilot({ project, worldGroupId })
  const candidateUpdateState = copilot.candidateUpdateState ?? EMPTY_CANDIDATE_UPDATE_STATE
  const quarantinedCount = copilot.quarantinedCandidates?.length ?? 0
  const hasUpdatingCandidate = copilot.pendingCandidates.some(candidate => (
    candidate.event.id != null
    && candidateUpdateState[candidate.event.id] === 'updating'
  ))
  const hasFailedCandidate = copilot.pendingCandidates.some(candidate => (
    candidate.event.id != null
    && candidateUpdateState[candidate.event.id] === 'failed'
  ))
  const hasGuardedCandidate = hasUpdatingCandidate || hasFailedCandidate
  const guard = useMemo<MasterCopilotGuardState>(() => {
    const reason = copilot.busy
      ? 'busy'
      : quarantinedCount > 0
        ? 'quarantined'
        : hasUpdatingCandidate
          ? 'updating'
          : hasFailedCandidate
            ? 'failed'
            : null
    return { active: reason != null || hasGuardedCandidate, reason }
  }, [copilot.busy, hasFailedCandidate, hasGuardedCandidate, hasUpdatingCandidate, quarantinedCount])
...
export default function WorkspacePage() {
  ...
  const [copilotGuard, setCopilotGuard] = useState<MasterCopilotGuardState>({ active: false, reason: null })
  ...
  const updateCopilotGuard = useCallback((next: MasterCopilotGuardState) => {
    setCopilotGuard(current => (
      current.active === next.active && current.reason === next.reason ? current : next
    ))
  }, [])
  ...
  const copilotGuardMessage = (reason: MasterCopilotGuardReason): string => {
    switch (reason) {
      case 'updating':
        return agentT('candidate.saving')
      case 'failed':
        return agentT('candidate.saveFailed')
      case 'quarantined':
        return agentT('candidate.quarantinedRecovery')
      case 'busy':
      default:
        return agentT('tasks.planning')
    }
  }

  const panelCloseButtonIsGuarded = () => {
    if (!showCopilot) return false
    // The panel owns the per-candidate update queue. Its disabled close
    // control covers a save/quarantine transition not yet visible to the
    // outer hook instance.
    return Boolean(
      workspaceRootRef.current?.querySelector<HTMLButtonElement>(
        'aside[aria-label] header button[aria-label]:disabled',
      ),
    )
  }

  const copilotCloseIsGuarded = () => copilotGuard.active || panelCloseButtonIsGuarded()

  const reportCopilotGuard = () => {
    toast.info(copilotGuardMessage(copilotGuard.reason ?? 'busy'))
  }

  const handleCopilotClose = () => {
    if (copilotCloseIsGuarded()) {
      reportCopilotGuard()
      setShowCopilot(true)
      return
    }
  ...
  return (
    <div ref={workspaceRootRef} className="h-screen bg-bg-base flex overflow-hidden">
      <WorkspaceCopilotGuard
        project={project}
        worldGroupId={copilotWorldGroupId}
        onChange={updateCopilotGuard}
      />
      {/* 左侧导航 */}
      <Sidebar
        active={activeModule}
      ...
      {(showCopilot || copilotGuard.active) && (
        <Suspense fallback={(
