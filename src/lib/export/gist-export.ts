...
export async function exportToGist(
  ...
  const body = {
    description: getT()('errors-lib:export.gistDescription', { name: data.project.name, date: new Date().toLocaleString('zh-CN') }),
    public: false,
    files: {
      [filename]: { content },
    },
  ...
  if (!response.ok) {
    ...
    throw new Error(err.message ?? getT()('errors-lib:export.gistApiError', { status: response.status }))
  }
...
export async function listStoryforgeGists(pat: string): Promise<GistBackupMeta[]> {
  ...
  if (!response.ok) throw new Error(getT()('errors-lib:export.gistApiError', { status: response.status }))
...
export async function importFromGist(gistId: string, pat: string, sha?: string): Promise<ProjectExportData> {
  ...
  if (!file) throw new Error(getT()('errors-lib:export.gistMissingBackupFile'))
...
export async function validateGitHubPAT(pat: string): Promise<string> {
  ...
  if (!response.ok) throw new Error(getT()('errors-lib:export.gistPatInvalid'))
  const json = await response.json()
