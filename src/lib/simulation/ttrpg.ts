...
import { buildSimulationNarrativeLanguageDirective } from './npc-evolution'
...
/** 遭遇候选中跟随项目内容语言的叙事字段；participantKeys 是实体键，不翻译。 */
export const TTRPG_ENCOUNTER_NARRATIVE_FIELDS = ['title', 'description'] as const

/**
 * GM 回合候选中跟随项目内容语言的叙事字段。
 * Oracle 修正（fix-6 复审）：runtime 把 check.skill 当作 ≤120 字符的自由散文
 * 接受，属于面向作者的文本，纳入语言契约；check.expression（骰子表达式）、
 * check.dc（数值）、actorKey/nextActorKey（实体键）仍属协议，保持原样。
 */
export const TTRPG_GM_NARRATIVE_FIELDS = [
  'narrative',
  'check.skill',
  'check.reason',
  'outcomes.success',
...
export function buildTtrpgEncounterPrompt(input: {
  ...
  if (input.contentLanguage) {
    system.push(buildSimulationNarrativeLanguageDirective(input.contentLanguage, TTRPG_ENCOUNTER_NARRATIVE_FIELDS))
  }
  return [
    {
      role: 'system',
...
export function buildTtrpgGmPrompt(input: {
  ...
  if (input.contentLanguage) {
    system.push(buildSimulationNarrativeLanguageDirective(input.contentLanguage, TTRPG_GM_NARRATIVE_FIELDS))
  }
  return [
    {
      role: 'system',
