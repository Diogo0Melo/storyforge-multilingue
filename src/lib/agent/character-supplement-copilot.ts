...
function buildMessages(input: CharacterSupplementCopilotInputV1): ChatMessage[] {
  const requested = input.snapshot.request.dimensions.map(key => {
    ...
    return `- ${key}（${getDimensionLabel(dimension.key)}）`
  }).join('\n')
  const jsonShape = Object.fromEntries(input.snapshot.request.dimensions.map(key => [key, 'content in the project content language']))
  return [{
    role: 'system',
    content: [
      ...
      '每个请求字段必须返回一个具体、非空、可直接保存的字符串，并遵循项目 contentLanguage。',
