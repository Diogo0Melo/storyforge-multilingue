...
function retryCauses(value: unknown, label: string): AgentTeamRetryCause[] {
  if (!Array.isArray(value)) fail(`${label} 必须是数组`)
  const result: AgentTeamRetryCause[] = []
...
function budgetEvidence(value: unknown, label: string): AgentTeamBudgetEvidence {
  ...
  const retryCauseList = retryCauses(value.retryCauses, `${label}.retryCauses`)
  const causes = retryCauses(value.causes, `${label}.causes`)
  if (JSON.stringify(retryCauseList) !== JSON.stringify(causes)) {
    fail(`${label}.causes 必须与 retryCauses 一致`)
  }
  const evidence = {
    profile,
    maxTokens: readInteger(value.maxTokens, `${label}.maxTokens`),
    maxCalls: readInteger(value.maxCalls, `${label}.maxCalls`),
    maxSemanticRetries: readInteger(value.maxSemanticRetries, `${label}.maxSemanticRetries`),
    ...
    semanticRetries: readInteger(value.semanticRetries, `${label}.semanticRetries`),
    retryCauses: retryCauseList,
  ...
  if (
    ...
    || evidence.maxSemanticRetries !== policy.maxSemanticRetries
    || evidence.maxCanonRetries !== policy.maxCanonRetries
    || evidence.usedTokens > policy.maxTokens
    || evidence.calls > policy.maxCalls
    || evidence.semanticRetries > policy.maxSemanticRetries
    || evidence.canonRetries > policy.maxCanonRetries
    || evidence.canonRetries > evidence.semanticRetries
    || evidence.retryCauses.length === 0 && evidence.semanticRetries > 0
    || evidence.retryCauses.length > 0 && evidence.semanticRetries === 0
...
function budgetAtLeast(next: AgentTeamBudgetEvidence, previous: AgentTeamBudgetEvidence): boolean {
  const retryCausesAreMonotonic = previous.retryCauses.every((cause, index) => (
    next.retryCauses[index] === cause
  ))
  return next.profile === previous.profile
    && next.maxTokens === previous.maxTokens
    && next.maxCalls === previous.maxCalls
    && next.maxSemanticRetries === previous.maxSemanticRetries
    && next.maxCanonRetries === previous.maxCanonRetries
    && next.usedTokens >= previous.usedTokens
    && next.calls >= previous.calls
    && next.semanticRetries >= previous.semanticRetries
    && retryCausesAreMonotonic
