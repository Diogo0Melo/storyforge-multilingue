...
/**
 * Revise an author-edited detailed-outline candidate without routing it through
 * the Master Agent candidate updater. Detailed-outline candidates use their own
 * RunContract step identity, so the generic Master payload validator must not
 * be used here.
 */
export async function reviseDetailedOutlineGenerationCandidateV1(input: {
  scope: WorkspaceScope
  eventId: number
  candidate: DetailedOutlineGenerationCandidateV1
  output: string
  creativeArtifact?: CreativeArtifactV1
}): Promise<DetailedOutlineGenerationCandidateV1> {
  if (!isCandidate(input.candidate)) throw new Error('细纲生成候选 durable evidence 不匹配。')
  const outputHash = await hashCanonicalValue(input.output)
  const creativeArtifact = input.creativeArtifact ?? input.candidate.creativeArtifact
  const draft: DetailedOutlineGenerationCandidateV1 = {
    ...input.candidate,
    output: input.output,
    outputHash,
    ...(creativeArtifact ? { creativeArtifact } : {}),
    durable: {
      ...input.candidate.durable,
      candidateHash: '',
    },
  }
  const candidateHash = await hashDetailedOutlineGenerationCandidateV1(draft)
  const candidate: DetailedOutlineGenerationCandidateV1 = {
    ...draft,
    durable: { ...draft.durable, candidateHash },
  }

  await db.transaction(
    'rw',
    agentRunScopeTransactionTablesV1(
      input.candidate.durable.runId,
      db.agentEvents,
      db.agentRuns,
      db.agentRunEvents,
    ),
    async () => {
      const event = await db.agentEvents.get(input.eventId)
      if (
        !event
        || event.kind !== 'candidate'
        || !await assertRecordInScope(input.scope, 'agentEvents', event, { owner: 'work' })
      ) throw new Error('细纲生成候选事件不存在或越界。')
      let payload: Record<string, unknown>
      try {
        const parsed = JSON.parse(event.payload) as unknown
        if (!isRecord(parsed)) throw new Error('细纲生成候选 payload 不是对象。')
        payload = parsed
      } catch (error) {
        throw new Error(error instanceof Error ? error.message : '细纲生成候选 payload 已损坏。')
      }
      if (
        payload.runId !== input.candidate.durable.runId
        || payload.runStepId !== input.candidate.durable.stepId
        || payload.candidateHash !== input.candidate.durable.candidateHash
      ) throw new Error('细纲生成候选与 durable 运行证据不匹配。')

      const snapshot = await readVerifiedAgentRunInTransactionV1(
        input.scope,
        input.candidate.durable.runId,
      )
      const step = snapshot.projection.steps[input.candidate.durable.stepId]
      if (
        !step
        || step.status !== 'awaiting_confirmation'
        || step.attempt !== input.candidate.durable.attempt
        || step.candidateHash !== input.candidate.durable.candidateHash
      ) throw new Error('细纲生成候选不在等待确认状态。')

      const revisedEvent = parseAgentRunEventV1({
        version: 1,
        runId: snapshot.run.id,
        ...
        payload: {
          ...
          previousCandidateHash: input.candidate.durable.candidateHash,
          candidateHash,
        },
      })
      await appendPrivilegedAgentRunEventInTransactionV1(snapshot, revisedEvent)
      await db.agentEvents.update(input.eventId, {
