...
async function executeNode(input: {
  ...
  if (node.kind === 'source.context') {
    ...
    if (!sourceKeys.length && !ragEntryKeys.length) {
      throw new Error(i18n.t('errors:nodeFlow.noSourceSelected'))
    }
  ...
  if (node.kind === 'generation.freeform') {
    ...
    if (!instruction) throw new Error(i18n.t('errors:nodeFlow.missingInstruction'))
...
export async function runNodeFlow(input: {
  ...
  if (input.flow.id == null) throw new Error(i18n.t('errors:nodeFlow.saveFirst'))
...
async function readStoredRun(runId: number): Promise<{
  ...
  if (!run) throw new Error(i18n.t('errors:nodeFlow.runNotFound'))
  const flow = await db.nodeFlows.get(run.flowId)
  if (!flow || flow.projectId !== run.projectId) throw new Error(i18n.t('errors:nodeFlow.flowNotFound'))
  return {
    run,
    flow,
...
export async function updateNodeRunOutput(input: {
  ...
  if (!result) throw new Error(i18n.t('errors:nodeFlow.noEditableOutput'))
  const results = {
    ...stored.results,
    [input.nodeId]: {
      ...result,
...
export async function adoptNodeRunOutput(input: {
  ...
  if (target === 'world-origin') {
    ...
    if (
      ...
      throw new Error(i18n.t('errors:nodeFlow.worldOriginWriteFailed'))
    }
    message = i18n.t('errors:nodeFlow.worldOriginWriteSuccess')
  } else {
    ...
    if (duplicate) throw new Error(i18n.t('errors:nodeFlow.characterDuplicate', { name: candidate.name }))
    const adopted = await adopt({
      projectId: stored.flow.projectId,
      worldGroupId: stored.flow.worldGroupId ?? null,
      target: 'characters',
    ...
    if (
      ...
      throw new Error(i18n.t('errors:nodeFlow.characterWriteFailed'))
    }
    message = i18n.t('errors:nodeFlow.characterAdded', { name: candidate.name })
  }
