...
请将上述世界观浓缩为 200-400 字的精华摘要，后续 AI 写作时会作为核心上下文参考。{{/if}}`,
    ...
    parameters: [
      { key: 'tone', label: '基调', labelKey: 'promptParams.tone.label', type: 'select',
        options: ['严肃', '史诗', '抒情', '硬核', '轻奇幻'],
        optionLabelKeys: [
          'promptParams.tone.options.serious',
          'promptParams.tone.options.epic',
          'promptParams.tone.options.lyrical',
          'promptParams.tone.options.hardcore',
          'promptParams.tone.options.lightFantasy',
        ],
        default: '严肃', description: '影响世界观叙述风格', descriptionKey: 'promptParams.tone.worldviewDescription', optional: true },
      { key: 'detailLevel', label: '详尽度', labelKey: 'promptParams.detailLevel.label', type: 'select',
        options: ['简略', '中等', '详尽'],
        optionLabelKeys: [
          'promptParams.detailLevel.options.brief',
          'promptParams.detailLevel.options.medium',
          'promptParams.detailLevel.options.detailed',
        ],
        default: '中等', description: '影响输出长度', descriptionKey: 'promptParams.detailLevel.lengthDescription', optional: true },
    ],
    isActive: true,
  },
...
用户要求：{{userHint}}{{/if}}`,
    ...
    parameters: [
      { key: 'archetype', label: '原型', labelKey: 'promptParams.archetype.label', type: 'select',
        options: ['普通人', '天才', '反英雄', '小人物逆袭', '隐忍蛰伏', '世家子弟', '废柴重生', '神秘高人'],
        optionLabelKeys: [
          'promptParams.archetype.options.ordinary',
          'promptParams.archetype.options.genius',
          'promptParams.archetype.options.antiHero',
          'promptParams.archetype.options.underdog',
          'promptParams.archetype.options.patient',
          'promptParams.archetype.options.noble',
          'promptParams.archetype.options.reborn',
          'promptParams.archetype.options.mysterious',
        ],
        default: '普通人', description: '角色的性格基底类型', descriptionKey: 'promptParams.archetype.description', optional: true },
      { key: 'detailLevel', label: '详尽度', labelKey: 'promptParams.detailLevel.label', type: 'select',
        options: ['简略', '中等', '详尽'],
        optionLabelKeys: [
          'promptParams.detailLevel.options.brief',
    ...
    parameters: [
      {
        key: 'tone',
        label: '基调',
        labelKey: 'promptParams.tone.label',
        type: 'select',
        options: ['严肃', '轻松', '幽默', '沉重', '抒情', '紧张', '热血'],
        optionLabelKeys: [
          'promptParams.tone.options.serious',
          'promptParams.tone.options.relaxed',
          'promptParams.tone.options.humorous',
          'promptParams.tone.options.heavy',
          'promptParams.tone.options.lyrical',
          'promptParams.tone.options.tense',
          'promptParams.tone.options.passionate',
        ],
        default: '严肃',
        description: '影响整体语言风格',
        descriptionKey: 'promptParams.tone.styleDescription',
        optional: true,
      },
      {
        key: 'pace',
        label: '节奏',
        labelKey: 'promptParams.pace.chapterLabel',
        type: 'select',
        options: ['慢', '中', '快', '极快'],
        optionLabelKeys: [
        ...
        descriptionKey: 'promptParams.pace.chapterDescription',
        optional: true,
      },
      {
        key: 'chapterLength',
        label: '目标字数',
        labelKey: 'promptParams.chapterLength.label',
        type: 'slider',
        min: 800,
        max: 6000,
        ...
        descriptionKey: 'promptParams.chapterLength.description',
        optional: true,
      },
    ],
    isActive: true,
...
用户额外要求：{{userHint}}{{/if}}`,
    ...
    parameters: [
      ...
      { key: 'pace', label: '节奏', labelKey: 'promptParams.pace.chapterLabel', type: 'select',
        options: ['慢', '中', '快', '极快'],
        optionLabelKeys: [
          'promptParams.pace.options.slow',
          'promptParams.pace.options.medium',
...
{{text}}`,
    ...
    parameters: [
      { key: 'style', label: '风格', labelKey: 'promptParams.style.label', type: 'select',
        options: ['口语化', '文艺', '精炼', '华丽', '冷峻', '温润'],
        optionLabelKeys: [
          'promptParams.style.options.colloquial',
          'promptParams.style.options.literary',
          'promptParams.style.options.concise',
          'promptParams.style.options.ornate',
          'promptParams.style.options.cold',
          'promptParams.style.options.gentle',
        ],
...
{{text}}`,
    ...
    parameters: [
      { key: 'expandRatio', label: '扩写倍数', labelKey: 'promptParams.expandRatio.label', type: 'select',
        options: ['1.5x', '2x', '3x'], default: '2x',
        description: '相对原文的字数倍数', descriptionKey: 'promptParams.expandRatio.description', optional: true },
      { key: 'addType', label: '主要增加', labelKey: 'promptParams.addType.label', type: 'select',
        options: ['心理描写', '环境氛围', '对话铺陈', '动作细节', '感官描写'],
        optionLabelKeys: [
          'promptParams.addType.options.psychology',
          'promptParams.addType.options.atmosphere',
          'promptParams.addType.options.dialogue',
          'promptParams.addType.options.action',
          'promptParams.addType.options.sensory',
        ],
  ...
  // 10. 章节-去 AI 味
  {
    ...
    variables: ['text'],
    parameters: [
      { key: 'aggressiveness', label: '改写力度', labelKey: 'promptParams.aggressiveness.label', type: 'select',
        options: ['轻度', '中度', '激进'],
        optionLabelKeys: [
          'promptParams.aggressiveness.options.light',
          'promptParams.aggressiveness.options.moderate',
          'promptParams.aggressiveness.options.aggressive',
        ],
        default: '中度',
        description: '激进=可能改变句式结构；轻度=只换用词', descriptionKey: 'promptParams.aggressiveness.description', optional: true },
    ],
    isActive: true,
  },
...
请建议 3-5 个精心设计的伏笔，每个包含名称、类型、描述、埋设方式和回收建议。`,
    ...
    parameters: [
      { key: 'density', label: '密度', labelKey: 'promptParams.density.label', type: 'select',
        options: ['稀疏', '中等', '密集'],
        optionLabelKeys: [
          'promptParams.density.options.sparse',
          'promptParams.density.options.medium',
          'promptParams.density.options.dense',
        ],
        default: '中等',
        description: '稀疏=只埋核心；密集=每条线都呼应', descriptionKey: 'promptParams.density.description', optional: true },
      { key: 'count', label: '建议数量', labelKey: 'promptParams.count.label', type: 'slider',
        min: 1, max: 12, step: 1, default: 5,
        description: '不指定则默认 3-5 个', descriptionKey: 'promptParams.count.description', optional: true },
    ],
    isActive: true,
  },
    ...
    parameters: [
      { key: 'tone', label: '基调', labelKey: 'promptParams.tone.label', type: 'select',
        ...
        optionLabelKeys: [
          ...
          'promptParams.tone.options.passionate',
          'promptParams.tone.options.lyrical',
          'promptParams.tone.options.dark',
          'promptParams.tone.options.warm',
          'promptParams.tone.options.fateful',
        ],
        default: '严肃', optional: true },
      { key: 'detailLevel', label: '详尽度', labelKey: 'promptParams.detailLevel.label', type: 'select',
        options: ['一句话', '简略', '中等', '详尽'],
        optionLabelKeys: [
          'promptParams.detailLevel.options.oneLiner',
          'promptParams.detailLevel.options.brief',
          'promptParams.detailLevel.options.medium',
          'promptParams.detailLevel.options.detailed',
        ],
...
用户补充：{{userHint}}{{/if}}`,
    ...
    parameters: [
      { key: 'strictness', label: '约束力度', labelKey: 'promptParams.strictness.label', type: 'select',
        options: ['宽松', '中等', '严格'],
        optionLabelKeys: [
          'promptParams.strictness.options.lenient',
          'promptParams.strictness.options.medium',
          'promptParams.strictness.options.strict',
        ],
        default: '中等',
        description: '严格=禁忌多 / 宽松=以建议为主', descriptionKey: 'promptParams.strictness.description', optional: true },
    ],
    isActive: true,
  },
