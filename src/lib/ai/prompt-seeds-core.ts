...
export const CORE_PROMPT_SEEDS: PromptSeed[] = [
  // 1. 世界观-单维度生成
  {
    ...
    descriptionKey: 'prompt.seed.worldviewDimension.desc',
...
请将上述世界观浓缩为 200-400 字的精华摘要，后续 AI 写作时会作为核心上下文参考。{{/if}}`,
    ...
    parameters: [
      { key: 'tone', label: '基调', labelKey: 'settings:prompt.parameters.tone', type: 'select',
        options: ['严肃', '史诗', '抒情', '硬核', '轻奇幻'],
        optionKeys: ['settings:prompt.parameters.toneOption.0', 'settings:prompt.parameters.toneOption.1', 'settings:prompt.parameters.toneOption.2', 'settings:prompt.parameters.toneOption.3', 'settings:prompt.parameters.toneOption.4'],
        default: '严肃', description: '影响世界观叙述风格', optional: true },
      { key: 'detailLevel', label: '详尽度', labelKey: 'settings:prompt.parameters.detailLevel', type: 'select',
        options: ['简略', '中等', '详尽'],
        optionKeys: ['settings:prompt.parameters.detailLevelOption.1', 'settings:prompt.parameters.detailLevelOption.2', 'settings:prompt.parameters.detailLevelOption.3'],
  ...
  // 2. 角色-完整生成
  {
    ...
    descriptionKey: 'prompt.seed.characterFullDesign.desc',
...
用户要求：{{userHint}}{{/if}}`,
    ...
    parameters: [
      { key: 'archetype', label: '原型', labelKey: 'settings:prompt.parameters.archetype', type: 'select',
        options: ['普通人', '天才', '反英雄', '小人物逆袭', '隐忍蛰伏', '世家子弟', '废柴重生', '神秘高人'],
        optionKeys: [
          'settings:prompt.parameters.archetypeOption.0',
          'settings:prompt.parameters.archetypeOption.1',
          'settings:prompt.parameters.archetypeOption.2',
          'settings:prompt.parameters.archetypeOption.3',
          'settings:prompt.parameters.archetypeOption.4',
          'settings:prompt.parameters.archetypeOption.5',
          'settings:prompt.parameters.archetypeOption.6',
          'settings:prompt.parameters.archetypeOption.7',
        ],
  ...
  // 3. 角色-单维度补全
  {
    ...
    descriptionKey: 'prompt.seed.characterDimensionFill.desc',
    systemPrompt: CHARACTER_SYSTEM,
...
请为这个角色丰富"{{dimension}}"这个维度的描写，要具体生动，约 200-400 字。`,
  ...
  // 4. 大纲-卷级
  {
    ...
    descriptionKey: 'prompt.seed.volumeOutline.desc',
...
用户补充要求：{{userHint}}{{/if}}`,
  ...
  // 5. 大纲-章节
  {
    ...
    descriptionKey: 'prompt.seed.chapterOutlineExpand.desc',
    systemPrompt: OUTLINE_SYSTEM,
...
用户补充要求：{{userHint}}{{/if}}`,
    ...
    parameters: [
      { key: 'pace', label: '节奏', labelKey: 'settings:prompt.parameters.pace', type: 'select',
        options: ['慢', '中', '快', '极快'],
        ...
        default: '中', optional: true },
      { key: 'wordsPerChapter', label: '每章字数', labelKey: 'settings:prompt.parameters.wordsPerChapter', type: 'number',
        min: 200, step: 100, default: 3000,
        description: '按你的更新习惯填（用于估算本卷章节数 = 卷字数 ÷ 每章字数）', optional: true },
      { key: 'chaptersPerVolume', label: '本卷章节数', labelKey: 'settings:prompt.parameters.chaptersPerVolume', type: 'slider',
  ...
  // 6. 章节-正文生成
  {
    ...
    descriptionKey: 'prompt.seed.longSerialDefault.desc',
    isDefault: true,
...
用户额外要求：{{userHint}}{{/if}}`,
    ...
    parameters: [
      {
        key: 'tone',
        label: '基调',
        labelKey: 'settings:prompt.parameters.tone',
        type: 'select',
        options: ['严肃', '轻松', '幽默', '沉重', '抒情', '紧张', '热血'],
        optionKeys: ['settings:prompt.parameters.toneOption.0', 'settings:prompt.parameters.toneOption.1', 'settings:prompt.parameters.toneOption.2', 'settings:prompt.parameters.toneOption.3', 'settings:prompt.parameters.toneOption.4', 'settings:prompt.parameters.toneOption.5', 'settings:prompt.parameters.toneOption.6'],
        default: '严肃',
      ...
      },
      {
        key: 'chapterLength',
        label: '目标字数',
        labelKey: 'settings:prompt.parameters.chapterLength',
        type: 'slider',
        min: 800,
        max: 6000,
  ...
  // 7. 章节-续写
  {
    ...
    descriptionKey: 'prompt.seed.chapterContinue.desc',
    systemPrompt: CHAPTER_SYSTEM,
...
用户额外要求：{{userHint}}{{/if}}`,
    ...
    parameters: [
      ...
      { key: 'pace', label: '节奏', labelKey: 'settings:prompt.parameters.pace', type: 'select',
        ...
        optionKeys: ['settings:prompt.parameters.paceOption.0', 'settings:prompt.parameters.paceOption.1', 'settings:prompt.parameters.paceOption.2', 'settings:prompt.parameters.paceOption.3'],
        default: '中', optional: true },
      { key: 'continueLength', label: '续写字数', labelKey: 'settings:prompt.parameters.continueLength', type: 'slider',
  ...
  // NS-1: 章节摘要 + continuity handoff 单次结构化抽取
  {
    ...
    descriptionKey: 'prompt.seed.chapterMemory.desc',
    isDefault: true,
...
请输出 JSON：`,
  ...
  // 8. 章节-润色
  {
    ...
    descriptionKey: 'prompt.seed.textPolish.desc',
...
{{text}}`,
    ...
    parameters: [
      { key: 'style', label: '风格', labelKey: 'settings:prompt.parameters.style', type: 'select',
        options: ['口语化', '文艺', '精炼', '华丽', '冷峻', '温润'],
        optionKeys: [
          'settings:prompt.parameters.styleOption.0',
          'settings:prompt.parameters.styleOption.1',
          'settings:prompt.parameters.styleOption.2',
          'settings:prompt.parameters.styleOption.3',
          'settings:prompt.parameters.styleOption.4',
          'settings:prompt.parameters.styleOption.5',
        ],
  ...
  // 9. 章节-扩写
  {
    ...
    descriptionKey: 'prompt.seed.textExpand.desc',
...
{{text}}`,
    ...
    parameters: [
      { key: 'expandRatio', label: '扩写倍数', labelKey: 'settings:prompt.parameters.expandRatio', type: 'select',
        options: ['1.5x', '2x', '3x'],
        optionKeys: [
          'settings:prompt.parameters.expandRatioOption.0',
          'settings:prompt.parameters.expandRatioOption.1',
          'settings:prompt.parameters.expandRatioOption.2',
        ],
        default: '2x',
        description: '相对原文的字数倍数', optional: true },
      { key: 'addType', label: '主要增加', labelKey: 'settings:prompt.parameters.addType', type: 'select',
        options: ['心理描写', '环境氛围', '对话铺陈', '动作细节', '感官描写'],
        optionKeys: [
          'settings:prompt.parameters.addTypeOption.0',
          'settings:prompt.parameters.addTypeOption.1',
          'settings:prompt.parameters.addTypeOption.2',
          'settings:prompt.parameters.addTypeOption.3',
          'settings:prompt.parameters.addTypeOption.4',
        ],
  ...
  // 10. 章节-去 AI 味
  {
    ...
    descriptionKey: 'prompt.seed.deAiRewrite.desc',
    systemPrompt: DEAI_SYSTEM,
    userPromptTemplate: `{{text}}`,
    variables: ['text'],
    parameters: [
      { key: 'aggressiveness', label: '改写力度', labelKey: 'settings:prompt.parameters.aggressiveness', type: 'select',
        options: ['轻度', '中度', '激进'],
        optionKeys: [
          'settings:prompt.parameters.aggressivenessOption.0',
          'settings:prompt.parameters.aggressivenessOption.1',
          'settings:prompt.parameters.aggressivenessOption.2',
        ],
        default: '中度',
  ...
  // 11. 伏笔-生成
  {
    ...
    descriptionKey: 'prompt.seed.foreshadowSuggest.desc',
...
请建议 3-5 个精心设计的伏笔，每个包含名称、类型、描述、埋设方式和回收建议。`,
    ...
    parameters: [
      { key: 'density', label: '密度', labelKey: 'settings:prompt.parameters.density', type: 'select',
        options: ['稀疏', '中等', '密集'],
        optionKeys: [
          'settings:prompt.parameters.densityOption.0',
          'settings:prompt.parameters.densityOption.1',
          'settings:prompt.parameters.densityOption.2',
        ],
        default: '中等',
        description: '稀疏=只埋核心；密集=每条线都呼应', optional: true },
      { key: 'count', label: '建议数量', labelKey: 'settings:prompt.parameters.count', type: 'slider',
  ...
  // 11.5 角色关系自动提取（Phase 30.2）
  {
    ...
    descriptionKey: 'prompt.seed.characterRelationExtract.desc',
...
{{/if}}请分析上述文本，提取所有角色之间的关系，输出 JSON 数组。`,
  ...
  // 12. 概念地图-SVG
  {
    ...
    descriptionKey: 'prompt.seed.conceptMapSvg.desc',
...
请生成 SVG 概念地图代码。`,
  ...
  // 13. 概念地图-外部图像 prompt（无 system，输出纯字符串）
  {
    ...
    descriptionKey: 'prompt.seed.mapImagePrompt.desc',
    systemPrompt: '',
  ...
  // 14. 故事设计-整体生成（Phase 8）
  {
    ...
    descriptionKey: 'prompt.seed.storyCore.desc',
...
用户补充说明：{{userHint}}{{/if}}`,
    ...
    parameters: [
      { key: 'tone', label: '基调', labelKey: 'settings:prompt.parameters.tone', type: 'select',
        ...
        optionKeys: ['settings:prompt.parameters.toneOption.0', 'settings:prompt.parameters.toneOption.1', 'settings:prompt.parameters.toneOption.2', 'settings:prompt.parameters.toneOption.3', 'settings:prompt.parameters.toneOption.4', 'settings:prompt.parameters.toneOption.5'],
        default: '严肃', optional: true },
      { key: 'detailLevel', label: '详尽度', labelKey: 'settings:prompt.parameters.detailLevel', type: 'select',
        options: ['一句话', '简略', '中等', '详尽'],
        optionKeys: ['settings:prompt.parameters.detailLevelOption.0', 'settings:prompt.parameters.detailLevelOption.1', 'settings:prompt.parameters.detailLevelOption.2', 'settings:prompt.parameters.detailLevelOption.3'],
        default: '中等', optional: true },
    ],
    isActive: true,
  },
  ...
  // 15. 创作规则-生成（Phase 8）
  {
    ...
    descriptionKey: 'prompt.seed.creativeRules.desc',
...
用户补充：{{userHint}}{{/if}}`,
    ...
    parameters: [
      { key: 'strictness', label: '约束力度', labelKey: 'settings:prompt.parameters.strictness', type: 'select',
        options: ['宽松', '中等', '严格'],
        optionKeys: [
          'settings:prompt.parameters.strictnessOption.0',
          'settings:prompt.parameters.strictnessOption.1',
          'settings:prompt.parameters.strictnessOption.2',
        ],
        default: '中等',
