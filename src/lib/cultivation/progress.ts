...
/** Ownership rejection messages for the upstream Work/World scope guards. */
export const CULTIVATION_SCOPE_ERROR_KEYS = {
  chapterNotInWork: 'errors-lib:cultivationScope.chapterNotInWork',
  characterNotInWorld: 'errors-lib:cultivationScope.characterNotInWorld',
  systemNotInWorld: 'errors-lib:cultivationScope.systemNotInWorld',
} as const
...
export async function acceptCultivationProgressCandidate(args: {
  ...
  if (!await assertRecordInScope(scope, 'chapters', chapter, { owner: 'work' })) {
    throw new Error(t(CULTIVATION_SCOPE_ERROR_KEYS.chapterNotInWork))
  }
  if (!await assertRecordInScope(scope, 'characters', character, { owner: 'world' })) {
    throw new Error(t(CULTIVATION_SCOPE_ERROR_KEYS.characterNotInWorld))
  }
  if (!await assertRecordInScope(scope, 'cultivationSystems', system, { owner: 'world' })) {
    throw new Error(t(CULTIVATION_SCOPE_ERROR_KEYS.systemNotInWorld))
  }
