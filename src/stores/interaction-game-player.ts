...
async function assertInteractionSession(scope: WorkspaceScope, sessionId: number): Promise<SimulationSession> {
  const stored = await db.simulationSessions.get(sessionId)
  if (!stored || stored.kind !== 'chatgame') throw new Error('该存档不是角色互动存档。')
  if (stored.gameReleaseId == null) throw new Error('该存档是 CHATGAME-1 旧存档，仅保留只读兼容。')
...
export const useInteractionGamePlayerStore = create<InteractionGamePlayerState>((set, get) => {
  ...
  return {
    ...
    saveCheckpoint: async name => withBusy(async () => {
      ...
      await assertInteractionSession(scope, id)
    ...
    remove: async sessionId => withBusy(async () => {
      const scope = get().scope
      if (!scope) throw new Error('请先加载工作区。')
      await assertInteractionSession(scope, sessionId)
