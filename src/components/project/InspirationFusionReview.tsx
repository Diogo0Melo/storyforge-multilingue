...
export default function InspirationFusionReview({
  ...
  const getSourceLabel = (kind: string) => {
    switch (kind) {
      case 'author': return t('fusion.sourceAuthor')
      case 'reference': return t('fusion.sourceReference')
      case 'research': return t('fusion.sourceResearch')
      default: return t('fusion.sourceOther')
    }
  }
  ...
  return (
    <section className="space-y-3 rounded-lg border border-border bg-bg-surface p-3">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-medium text-text-primary">{t('fusion.title')}</h3>
          <p className="mt-0.5 text-xs text-text-muted">
            {t('fusion.instruction')}
          </p>
        </div>
        <span className="flex items-center gap-1 text-xs text-text-muted">
          <History className="h-3.5 w-3.5" />
          {t('fusion.versionCount', { count: modeVersions.length })}
        </span>
      </div>
          ...
          {fragments.map(fragment => {
            ...
            >
              <input
                ...
                aria-label={`${t('fusion.selectFragment')} ${fragment.label || fragment.text.slice(0, 20)}`}
              />
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 text-[11px] text-text-muted">
                  <span className="rounded bg-bg-elevated px-1.5 py-0.5">
                    {getSourceLabel(fragment.sourceKind)}
                  </span>
              ...
              <button
                ...
                title={referenced
                  ? t('fusion.referencedTooltip')
                  : t('fusion.removeTooltip')}
                aria-label={t('fusion.removeLabel')}
              >
      ...
      {pendingDiff && (
        <div className="space-y-2 rounded-lg border border-amber-500/30 bg-amber-500/5 p-3">
          <div className="flex items-center gap-1.5 text-sm font-medium text-amber-400">
            ...
            {t('fusion.pendingHeader')}
          </div>
          <p className="text-xs text-text-muted">
            {t('fusion.pendingDesc')}
          </p>
          {pendingDiff.length === 0 ? (
            <p className="text-xs text-text-secondary">{t('fusion.noDiff')}</p>
          ) : (
                ...
                <div key={diff.path} className="rounded border border-border bg-bg-base p-2 text-xs">
                  ...
                  <div className="grid gap-1 md:grid-cols-2">
                    <div className="rounded bg-red-500/5 p-1.5 text-text-muted">
                      <span className="mb-0.5 block text-[10px] text-red-400">{t('fusion.oldVersion')}</span>
                      {diff.before || t('fusion.none')}
                    </div>
                    <div className="rounded bg-green-500/5 p-1.5 text-text-secondary">
                      <span className="mb-0.5 block text-[10px] text-green-400">{t('fusion.newVersion')}</span>
                      {diff.after || t('fusion.deleted')}
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {confirming ? t('fusion.saving') : t('fusion.confirmVersion')}
            </button>
            <button
              onClick={onDiscard}
              disabled={confirming}
