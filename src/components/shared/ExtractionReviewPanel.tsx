...
export default function ExtractionReviewPanel<T>({
  ...
  const writeLabel = confirmLabel ?? t('extractionReview.confirmWrite', { count: selected.size })
  const selectionDisabled = Boolean(loading || busy || selectionLocked)
  const actionBusy = Boolean(loading || busy)
  return (
    <div
      aria-busy={actionBusy}
      className="mt-3 rounded-xl border border-accent/30 bg-bg-surface p-3 space-y-3"
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          title={t('extractionReview.closeTitle')}
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      {(loading || busy) && (
        <div className="flex items-center gap-2 text-xs text-text-muted">
          ...
          {loading ? t('extractionReview.analyzing') : t('extractionReview.writing')}
        </div>
      )}
        ...
        <>
          <div className="max-h-72 overflow-y-auto space-y-2">
            {items.map((item, index) => (
              <label
                key={index}
                className={`flex items-start gap-2 rounded-lg border p-2.5 ${selectionDisabled ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'} ${
          ...
          <div className="flex justify-end">
            <button
              onClick={onConfirm}
              disabled={(!allowEmptyConfirm && selected.size === 0) || actionBusy}
