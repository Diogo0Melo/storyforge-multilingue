...
export default function PromptRunPanel({
  ...
  if (!tpl) {
    return (
      <div className="text-xs text-text-muted px-3 py-2 bg-bg-elevated rounded">
        ⚠ {t('promptRun.templateNotFound', { key: moduleKey })}
      </div>
    )
  }
  ...
  const handleSaveAs = async () => {
    const name = await dialog.prompt({
      title: t('promptRun.saveAsTitle'),
      defaultValue: t('promptRun.saveAsDefault', { name: tpl.name }),
      placeholder: t('promptRun.saveAsPlaceholder'),
    })
    if (!name) return
    ...
    if (cloned) {
      ...
      toast.success(t('promptRun.saveAsSuccess', { name }))
    }
  }
    ...
    <div className="bg-bg-elevated border border-border rounded-lg text-xs">
      ...
      {open && (
        <div className="border-t border-border p-3 space-y-3">
          {/* 参数区 */}
          {params.length === 0 ? (
            <p className="text-text-muted">{t('promptRun.noParams')}</p>
          ) : (
          ...
          <details className="border-t border-border pt-2" open={showAdvanced} onToggle={(e) => setShowAdvanced((e.target as HTMLDetailsElement).open)}>
            <summary className="cursor-pointer text-text-secondary hover:text-text-primary">
              {t('promptRun.advancedTitle')}
            </summary>
            <div className="mt-2 space-y-2">
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-text-muted">{t('promptRun.restore')}</span>
                  {systemOverride != null && (
                    <button onClick={() => onSystemOverrideChange(null)} className="text-accent hover:underline">{t('promptRun.restore')}</button>
                  )}
                </div>
                <textarea
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-text-muted">{t('promptRun.restore')}</span>
                  {userOverride != null && (
                    <button onClick={() => onUserOverrideChange(null)} className="text-accent hover:underline">{t('promptRun.restore')}</button>
                  )}
                </div>
                <textarea
          ...
          <div className="flex items-center justify-between pt-2 border-t border-border">
            ...
            >
              <RotateCcw className="w-3 h-3" /> {t('promptRun.reset')}
            </button>
            <button
            ...
            >
              <Save className="w-3 h-3" /> {t('promptRun.saveAsMine')}
            </button>
          </div>
        </div>
      )}
