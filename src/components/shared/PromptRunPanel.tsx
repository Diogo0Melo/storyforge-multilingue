...
export default function PromptRunPanel({
  ...
  // settings ns 经由 own-ns 钩子保证加载:本面板挂在创作区路由,用户可能从未访问过
  // 设置页,`shared` 钩子上跨 ns 调 `settings:*` 会因 bundle 未加载而回退中文(Gate 5 · S1)。
  const { t: tSettings } = useDomainT('settings')
  ...
  if (!tpl) {
    return (
      <div className="text-xs text-text-muted px-3 py-2 bg-bg-elevated rounded">
        {t('promptRunPanel.templateMissing', { moduleKey })}
      </div>
    )
  }
  ...
  // 系统种子显示名经 settings ns 解析;用户模板与未登记种子保留原文(Gate 5 · B2)
  const { name: displayName } = resolveSystemSeedDisplay(tSettings, 'prompt', tpl)

  /** 把当前覆盖另存为新的"我的"模板 */
  const handleSaveAs = async () => {
    const name = await dialog.prompt({
      title: t('promptRunPanel.saveAsTitle'),
      defaultValue: t('promptRunPanel.saveAsDefaultValue', { name: displayName }),
      placeholder: t('promptRunPanel.saveAsPlaceholder'),
    })
    if (!name) return
    ...
    if (cloned) {
      ...
      toast.success(t('promptRunPanel.saveAsSuccess', { name }))
    }
  }
    ...
    <div className="bg-bg-elevated border border-border rounded-lg text-xs">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-text-secondary">{t('promptRunPanel.currentTemplate')}</span>
          {/* P1-4:头部用 settings ns 解析后的显示名;未登记种子保留原文(seed-i18n 契约) */}
      ...
      {open && (
        <div className="border-t border-border p-3 space-y-3">
          {/* 参数区 */}
          {params.length === 0 ? (
            <p className="text-text-muted">{t('promptRunPanel.noParams')}</p>
          ) : (
          ...
          <details className="border-t border-border pt-2" open={showAdvanced} onToggle={(e) => setShowAdvanced((e.target as HTMLDetailsElement).open)}>
            <summary className="cursor-pointer text-text-secondary hover:text-text-primary">
              {t('promptRunPanel.advancedHeading')}
            </summary>
            <div className="mt-2 space-y-2">
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  {systemOverride != null && (
                    <button onClick={() => onSystemOverrideChange(null)} className="text-accent hover:underline">{t('promptRunPanel.restoreOverride')}</button>
                  )}
                </div>
                <textarea
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-text-muted">{t('promptRunPanel.userPromptLabel')}</span>
                  {userOverride != null && (
                    <button onClick={() => onUserOverrideChange(null)} className="text-accent hover:underline">{t('promptRunPanel.restoreOverride')}</button>
                  )}
                </div>
                <textarea
          ...
          <div className="flex items-center justify-between pt-2 border-t border-border">
            ...
            >
              <RotateCcw className="w-3 h-3" /> {t('promptRunPanel.reset')}
            </button>
            <button
            ...
            >
              <Save className="w-3 h-3" /> {t('promptRunPanel.saveAs')}
            </button>
          </div>
        </div>
      )}
...
function ParamControl({
  ...
  const { t } = useDomainT('shared')
  // settings ns 经由 own-ns 钩子保证加载(与主组件同理, Gate 5 · S1)
  const { t: tSettings } = useDomainT('settings')
    ...
    <div className="flex items-center gap-2">
      ...
      {param.optional && (
        <input
          ...
          title={t('promptRunPanel.enableParamTitle')}
        />
      )}
      <label className={`w-20 flex-shrink-0 ${enabled ? 'text-text-secondary' : 'text-text-muted line-through'}`}>
        {param.labelKey ? tSettings(param.labelKey, param.label) : param.label}
      </label>
      {param.type === 'select' && (
        <select
          value={String(shown)}
        ...
        >
          {(param.options || []).map((opt, idx) => (
            <option key={opt} value={opt}>
              {param.optionLabelKeys && param.optionLabelKeys[idx]
                ? tSettings(param.optionLabelKeys[idx], opt)
                : opt}
            </option>
          ))}
        </select>
