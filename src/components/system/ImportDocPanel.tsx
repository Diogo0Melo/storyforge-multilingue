...
export default function ImportDocPanel({ project, onNavigate }: Props) {
  ...
  const handleConfirmStart = async (importTarget: ImportTarget, selectedWorldGroupId?: number | null, depth?: import('../../lib/types').ReferenceAnalysisDepth) => {
    ...
    if (importTarget === 'project' && project.enableMultiWorld && selectedWorldGroupId == null) {
      toast.error(t('panel.selectTargetWorld'))
      return
    }
    closeConfirm()
    ...
    const sessionData: Omit<ImportSession, 'id' | 'createdAt' | 'updatedAt'> = {
      ...
      filename: filename || t('panel.unnamedDocument'),
      fileHash,
    ...
    if (importTarget === 'project' && volumeDetect?.hasVolumes) {
      try {
        ...
        useImportStatusStore.getState().pushActivity(
          'info',
          t('panel.createdVolumeSkeleton', { count: volumeDetect.volumes.length }),
        )
      } catch (err) {
  ...
  const handleReuseToProject = async () => {
    ...
    statusStore.pushActivity('info', t('panel.reuseProjectActivity', { filename: reusable.filename }))
    try {
      await applyProjectFromSession(project.id!, reusable, null, statusStore)
      statusStore.setPhase('done')
      clearReusable()
      toast.success(t('panel.reuseProjectSuccess'))
    } catch (err) {
  ...
  const handleReuseToReference = async (depth: ReferenceAnalysisDepth) => {
    ...
    statusStore.reset()
    statusStore.setPhase('preparing')
    statusStore.pushActivity('info', t('panel.reuseReferenceActivity', { filename: reusable.filename, depth: depth === 'deep' ? t('panel.deepLabel') : t('panel.shallowLabel') }))
    try {
  ...
  const handleResume = async () => {
    ...
    if (!hasChunkTexts(unfinished.id)) {
      toast.error(t('panel.blobRestoreFailedToast'))
      return
    }
    autoReportShown.current = null
    setReportSession(null)
  ...
  const handleResumeWithUploaded = async () => {
    ...
    if (newHash !== unfinished.fileHash) {
      const ok = await dialog.confirm({
        title: t('panel.uploadMismatchTitle'),
        message: t('panel.uploadMismatchMessage', { original: unfinished.fileHash, current: newHash }),
        confirmText: t('panel.uploadMismatchConfirm'),
        tone: 'danger',
      })
      if (!ok) return
    }
    ...
    if (p.length !== unfinished.totalChunks) {
      toast.error(t('panel.chunkMismatchToast', { new: p.length, original: unfinished.totalChunks }))
      return
    }
  ...
  const handleRetryFailed = async () => {
    ...
    if (!hasChunkTexts(reportSession.id)) {
      toast.error(t('panel.retryBlobLostToast'))
      return
    }
    setReportSession(null)
    autoReportShown.current = null
  ...
  const handleDiscardSession = async () => {
    if (!reportSession?.id) return
    const ok = await dialog.confirm({
      title: t('panel.discardSessionTitle'),
      message: t('panel.discardSessionMessage'),
      confirmText: t('panel.discardSessionConfirm'),
      tone: 'danger',
    })
    if (!ok) return
    clearChunkTexts(reportSession.id)
  ...
  const handleCancelCurrent = async () => {
    const ok = await dialog.confirm({
      title: t('panel.cancelTaskTitle'),
      message: t('panel.cancelTaskMessage'),
      confirmText: t('panel.cancelTaskConfirm'),
      tone: 'danger',
    })
  ...
  const handleOpenCodexReview = async (session: ImportSession) => {
    ...
    try {
      ...
      toast.error(t('panel.loadCategoryFailed', { error: error instanceof Error ? error.message : String(error) }))
    }
  }
  ...
  const handleConfirmCodex = async (
    ...
    if (project.enableMultiWorld && worldGroupId == null) {
      toast.error(t('panel.selectCodexTargetWorld'))
      return
    }
    try {
      ...
      toast.success(t('panel.codexReviewComplete', { imported: result.imported, updated: result.updated, skipped: result.skipped, detail }))
    } catch (error) {
      toast.error(t('panel.codexWriteFailed', { error: error instanceof Error ? error.message : String(error) }))
    }
  }
    ...
    <div className="max-w-4xl p-6 space-y-4">
      ...
      {unfinished && phase === 'idle' && (
        <ImportUnfinishedBanner
          ...
          onDiscard={async () => {
            const ok = await dialog.confirm({
              title: t('panel.discardUnfinishedTitle'),
              message: t('panel.discardUnfinishedMessage'),
              confirmText: t('panel.discardUnfinishedConfirm'),
              tone: 'danger',
            })
            if (!ok) return
