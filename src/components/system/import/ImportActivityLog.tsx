...
export default function ImportActivityLog() {
  ...
  const { t, lang } = useDomainT('system')
  if (activity.length === 0) return null

  return (
    <div className="bg-bg-surface border border-border rounded-xl p-4">
      <div className="flex items-center justify-between mb-2">
        <div className="text-sm font-semibold text-text-primary">{t('activityLog.heading')}</div>
        <div className="text-xs text-text-muted">{t('activityLog.recentCount', { count: activity.length })}</div>
      </div>
      <div className="max-h-64 overflow-y-auto space-y-1 pr-1">
        {activity.map(entry => {
          const Icon = ICONS[entry.level]
          const color = COLORS[entry.level]
          const time = new Date(entry.time).toLocaleTimeString(lang, { hour12: false })
          return (
