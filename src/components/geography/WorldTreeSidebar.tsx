  ...
  const handleAddChild = useCallback(
    async (parentId: number | null) => {
      ...
      await createNode({
        projectId,
        parentId,
        name: parentId ? t('worldTree.newChildWorldName') : t('worldTree.newWorldName'),
        description: '',
        sortOrder: siblingCount,
        icon: parentId ? '🌀' : '🌍',
      })
    },
    [projectId, nodes, createNode, t],
  )
    ...
    return (
      <div className="w-10 shrink-0 border-r border-border bg-bg-elevated flex flex-col items-center py-2 gap-1">
        <button
          ...
          title={t('worldTree.expandSidebar')}
        >
  ...
  return (
    <div className="w-40 shrink-0 border-r border-border bg-bg-elevated flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-2 py-1.5 border-b border-border">
        <span className="text-[10px] font-medium text-text-secondary">{t('worldTree.title')}</span>
        <div className="flex items-center gap-0.5">
          <button
            onClick={() => handleAddChild(null)}
            className="p-1 rounded hover:bg-bg-hover text-text-muted hover:text-accent transition-colors"
            title={t('worldTree.newRootWorld')}
          >
            <Plus className="w-3 h-3" />
          </button>
          <button
            ...
            title={t('worldTree.collapseSidebar')}
          >
      ...
      <div className="flex-1 overflow-y-auto py-0.5">
        {tree.length === 0 ? (
          <div className="px-2 py-4 text-center text-text-muted text-[10px]">{t('worldTree.empty')}</div>
        ) : (
          tree.map(node => (
            <TreeItem
              key={node.id}
      ...
      >
        ...
        <div className="hidden group-hover:flex items-center shrink-0">
          ...
          <button
            ...
            title={t('worldTree.newChildWorld')}
          >
          ...
          <button
            onClick={async (e) => {
              ...
              const ok = await dialog.confirm({
                title: t('worldTree.deleteConfirmTitle', { name: node.name }),
                message: t('worldTree.deleteConfirmMessage'),
                confirmText: t('worldTree.deleteConfirmButton'),
                tone: 'danger',
              })
              if (ok) onDelete(node.id!)
            }}
            className="p-0.5 rounded hover:bg-bg-base text-text-muted hover:text-red-400"
            title={t('worldTree.delete')}
          >
