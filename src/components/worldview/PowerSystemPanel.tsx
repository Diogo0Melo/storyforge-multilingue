  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-text-primary">⚡ {t('worldview.power.title')}</h2>
        {project.enableMultiWorld && <WorldGroupSwitcher />}
      </div>
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-text-secondary mb-1">{t('worldview.power.name')}</label>
          <input
            value={name}
            onChange={e => setName(e.target.value)}
            onBlur={handleSave}
            placeholder={t('worldview.power.namePlaceholder')}
        ...
        <div>
          <label className="block text-sm font-medium text-text-secondary mb-1">{t('worldview.power.description')}</label>
          <textarea
            value={description}
            onChange={e => setDescription(e.target.value)}
            onBlur={handleSave}
            placeholder={t('worldview.power.descPlaceholder')}
            rows={3}
        ...
        <div>
          <label className="block text-sm font-medium text-text-secondary mb-1">{t('worldview.power.levels')}</label>
          <textarea
            value={levels}
            onChange={e => setLevels(e.target.value)}
            onBlur={handleSave}
            placeholder={t('worldview.power.levelsPlaceholder')}
            rows={5}
            className="w-full p-3 bg-bg-surface border border-border rounded-lg text-text-primary text-sm resize-y focus:outline-none focus:border-accent"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-text-secondary mb-1">{t('worldview.power.rules')}</label>
          <textarea
            value={rules}
            onChange={e => setRules(e.target.value)}
            onBlur={handleSave}
            placeholder={t('worldview.power.rulesPlaceholder')}
            rows={4}
            className="w-full p-3 bg-bg-surface border border-border rounded-lg text-text-primary text-sm resize-y focus:outline-none focus:border-accent"
          />
        </div>
