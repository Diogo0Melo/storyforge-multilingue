  ...
  const addRow = (level: 'l1' | 'l2', placeholder: string) => (
    <div className="px-2 py-1.5 flex gap-1">
      ...
      <button
        ...
        aria-label={t(level === 'l1' ? 'rules.l1ConfirmAdd' : 'rules.l2ConfirmAdd')}
      >
      ...
      <button
        ...
        aria-label={t(level === 'l1' ? 'rules.l1CancelAdd' : 'rules.l2CancelAdd')}
      >
  ...
  return (
    <>
      <div className="w-48 shrink-0 bg-bg-elevated border-r border-border overflow-y-auto">
        ...
        {addingL1 ? addRow('l1', t('rules.l1Placeholder')) : (
          <button
          ...
          >
            <Plus className="w-3.5 h-3.5" /> {t('rules.l1AddButton')}
          </button>
        )}
      </div>

      <div className="w-52 shrink-0 border-r border-border overflow-y-auto">
        <div className="p-2 text-[10px] font-semibold text-text-muted uppercase tracking-wider px-3 pt-3">{t('rules.l2Category')}</div>
        <button
        ...
        >
          ...
          <span className="flex-1 truncate">{t('rules.l2Overview')}</span>
          ...
          >
            ...
            {node.isCustom && (
              <button
                ...
                aria-label={t('rules.l2DeleteAria', { label: node.label })}
              >
        ...
        {addingL2 ? addRow('l2', t('rules.l2Placeholder')) : (
          <button
            onClick={() => { setAddingL2(true); setNewNodeLabel('') }}
            className="w-full text-left px-3 py-2 text-xs text-text-muted hover:text-accent flex items-center gap-1.5 transition-colors"
          >
            <Plus className="w-3.5 h-3.5" /> {t('rules.l2AddButton')}
          </button>
        )}
      </div>
    </>
