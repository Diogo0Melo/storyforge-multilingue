...
import { buildCultivationProgressExpectedProjectionV1 } from '../../lib/cultivation/progress'
import {
...
export default function CultivationProgressPanel({ project }: { project: Project }) {
  ...
  const candidateTransitions = useMemo(() => {
    if (!extractCandidate) return []
    try {
      return buildCultivationProgressExpectedProjectionV1(
        extractCandidate.baseline,
        extractCandidate.events,
      ).formalItems.map(item => item.transition)
    } catch {
      return []
    }
  }, [extractCandidate])
        ...
        {extractCandidate && (
          <div className="space-y-2 pt-2">
            {extractCandidate.events.map((candidate, index) => {
              ...
              const transition = candidateTransitions[index]
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-text-primary">
                        ...
                        {transition && <span className="ml-2 text-[10px] text-accent">{t(TRANSITION_KEYS[transition])}</span>}
                      </p>
