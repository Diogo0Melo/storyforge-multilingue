...
const ACTION_LABEL_KEYS = {
  learn: 'knowledge.actionLearn' as const,
  mislearn: 'knowledge.actionMislearn' as const,
  forget: 'knowledge.actionForget' as const,
  correct: 'knowledge.actionCorrect' as const,
} satisfies Record<KnowledgeAction, string>

const STATUS_LABEL_KEYS = {
  candidate: 'knowledge.statusCandidate' as const,
  confirmed: 'knowledge.statusConfirmed' as const,
  rejected: 'knowledge.statusRejected' as const,
  'source-missing': 'knowledge.statusSourceMissing' as const,
  'invalid-range': 'knowledge.statusInvalidRange' as const,
} satisfies Record<KnowledgeEventStatus, string>
...
export default function KnowledgeLedgerPanel({ project, onShowFacts }: {
  ...
  const addCandidate = async () => {
    ...
    if (!character || !knowledgeKey.trim() || !statement.trim()) {
      setMessage(t('knowledge.validationCharacterRequired'))
      return
    }
    if (action === 'mislearn' && !belief.trim()) {
      setMessage(t('knowledge.validationMislearnBeliefRequired'))
      return
    }
    ...
    setMessage(result.written
      ? t('knowledge.adoptSuccessMsg')
      : t('knowledge.adoptSkippedMsg', { count: result.skipped }))
    if (result.written) {
      setKnowledgeKey('')
      setStatement('')
      setBelief('')
  ...
  const statusKeys = Object.keys(STATUS_LABEL_KEYS) as KnowledgeEventStatus[]
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between gap-3 mb-1">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold text-text-primary">{t('knowledge.title')}</h1>
        </div>
      ...
      <div className="mb-4 p-3 rounded-lg border border-border bg-bg-elevated/60 space-y-2">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          <select value={characterId} onChange={event => setCharacterId(event.target.value)}
            ...
            <option value="">{t('knowledge.characterSelectPlaceholder')}</option>
            {characters.map(character => <option key={character.id} value={character.id}>{character.name}</option>)}
          </select>
          <select value={action} onChange={event => setAction(event.target.value as KnowledgeAction)}
            className="px-2 py-1.5 text-xs rounded bg-bg-base border border-border text-text-primary">
            {(Object.keys(ACTION_LABEL_KEYS) as KnowledgeAction[]).map(key => {
              const actionKey = ACTION_LABEL_KEYS[key]
              return <option key={key} value={key}>{t(actionKey)}</option>
            })}
          </select>
          <select value={sourceChapterId} onChange={event => setSourceChapterId(event.target.value)}
            className="px-2 py-1.5 text-xs rounded bg-bg-base border border-border text-text-primary">
            <option value="">{t('knowledge.baselineSourceOption')}</option>
            {chapters.map(chapter => <option key={chapter.id} value={chapter.id}>{chapter.title}</option>)}
          </select>
        </div>
        <input value={knowledgeKey} onChange={event => setKnowledgeKey(event.target.value)}
          placeholder={t('knowledge.knowledgeKeyPlaceholder')}
        ...
        <textarea value={statement} onChange={event => setStatement(event.target.value)}
          placeholder={t('knowledge.statementPlaceholder')}
        ...
        {action === 'mislearn' && (
          <textarea value={belief} onChange={event => setBelief(event.target.value)}
            placeholder={t('knowledge.beliefPlaceholder')}
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => void addCandidate()}
            ...
            <Plus className="w-3.5 h-3.5" /> {t('knowledge.addCandidateButton')}
          </button>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {statusKeys.map(key => {
          const statusKey = STATUS_LABEL_KEYS[key]
          return (
            <button key={key} onClick={() => setStatus(key)}
              ...
              {t(statusKey)}{counts.get(key) ? t('knowledge.tabCountSuffix', { count: counts.get(key) }) : ''}
            </button>
          )
        })}
      </div>

      {loading && <p className="text-sm text-text-muted">{t('knowledge.loading')}</p>}
      {!loading && !rows.length && (() => {
        const emptyStatusKey = STATUS_LABEL_KEYS[status]
        return <p className="text-sm text-text-muted py-8 text-center">{t('knowledge.emptyState', { status: t(emptyStatusKey) })}</p>
      })()}
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-text-primary">
                ...
                <span className="text-text-muted"> · {(() => {
                  ...
                  return t('knowledge.actionValueLabel', { label: t(actionKey) })
                })()}</span>
              ...
              {event.belief && <p className="text-xs text-amber-300 mt-1">{t('knowledge.beliefPrefix')}{event.belief}</p>}
              <p className="text-[11px] text-text-muted mt-1">
                {t('knowledge.sourcePrefix')}{event.sourceChapterId == null
                  ? t('knowledge.sourceBaseline')
                  : chapters.find(chapter => chapter.id === event.sourceChapterId)?.title ?? t('knowledge.sourceChapterFallback', { id: event.sourceChapterId })}
              </p>
            </div>
            {REVIEWABLE.includes(event.status) && event.id != null && (
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => void confirmEvent(project.id!, event.id!)} title={t('knowledge.confirmEventTitle')}
                  className="p-1.5 text-emerald-400 hover:bg-emerald-500/15 rounded"><Check className="w-4 h-4" /></button>
                <button onClick={() => void rejectEvent(project.id!, event.id!)} title={t('knowledge.rejectEventTitle')}
