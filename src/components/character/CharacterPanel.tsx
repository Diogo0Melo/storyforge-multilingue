...
export default function CharacterPanel({ project, view = 'generator' }: Props) {
  ...
  const handleAdd = async () => {
    ...
    const id = await addCharacter({
      projectId: project.id!, name: t('panel.newCharacter'),
        ...
        {view === 'generator' && (
          <>
            <div className="relative">
              ...
              >
                <Plus className="w-4 h-4" /> {t('panel.add')} <ChevronDown className="w-3 h-3 ml-0.5" />
              </button>
              {showRolePicker && (
                <>
            ...
            <div className="flex items-center gap-2 flex-1">
              <CInput
                ...
                placeholder={t('panel.hintPlaceholder')}
              ...
              <div className="relative">
                <button
                  ...
                  title={t('panel.dimPickerTitle')}
                >
                  {t('panel.dimensionCount', { selected: genDims.size, total: CHARACTER_DIMENSIONS.length })} <ChevronDown className="w-3 h-3" />
                </button>
                {showDimPicker && (
                  <>
              ...
              >
                <Sparkles className="w-3.5 h-3.5" /> {t('panel.aiGenerate')}
              </button>
            </div>
          </>
        )}
        <span className="text-xs text-text-muted ml-auto">
          {view === 'main' ? t('panel.mainTitle') : t('panel.generatorTitle')} · {displayedChars.length}
        </span>
      </div>
      ...
      {view === 'generator' && (ai.output || ai.isStreaming || ai.error) && (
        <AIStreamOutput
          ...
          onAccept={async (text: string) => {
            ...
            const fallbackName = nameMatch?.[1]?.trim() || t('panel.aiGeneratedName')
