...
export default function LocationPanel({ project }: Props) {
  ...
  useEffect(() => {
    void resolveScopeLike(project.id!).then(scope => loadAll(scope))
  }, [project.id, loadAll])
  ...
  const handleAdd = useCallback(async (parentId: number | null = null) => {
    const id = await addLocation({
      ...
      // Persisted default entity name — Wave E will migrate to common.defaults.*
      name: t('defaults.newLocationName'),
      tags: '[]',
      description: '',
      significance: '',
      parentId,
  ...
  const handleExtractLocations = async () => {
    ...
    try {
      const generated = await generateLocationExtractionCandidateV1({
        ...
        onDurableBoundary: (_boundary, snapshot, callIndex) => {
          const total = Math.max(1, snapshot.contract.budget.maxModelCalls)
          setProgress({ done: callIndex < 0 ? 0 : Math.min(callIndex + 1, total), total })
        },
      })
          ...
          >
            ...
            {children.length > 0 && (
              <span className="text-[10px] text-text-muted bg-bg-elevated px-1.5 py-0.5 rounded shrink-0">
                {t('list.childCount', { count: children.length })}
              </span>
            )}
          </button>
          ...
          {isExpanded && (
            <div className="px-4 pb-4 space-y-3 border-t border-border pt-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-text-muted mb-1">{t('list.labelName')}</label>
                  <input
                    value={loc.name}
                ...
                <div>
                  <label className="block text-xs text-text-muted mb-1">{t('list.labelParent')}</label>
                  <select
                    value={loc.parentId ?? ''}
                    onChange={e => updateLocation(loc.id!, { parentId: e.target.value ? Number(e.target.value) : null })}
                    className="w-full px-2 py-1.5 bg-bg-base border border-border rounded text-sm text-text-primary focus:outline-none focus:border-accent"
                  >
                    <option value="">{t('list.parentTopLevel')}</option>
                    {locations
              ...
              {/* 标签 */}
              <div>
                <label className="block text-xs text-text-muted mb-1">{t('list.labelTags')}</label>
                <LocationTagPicker
                  selected={tags}
              ...
              {/* 描述 */}
              <div>
                <label className="block text-xs text-text-muted mb-1">{t('list.labelDescription')}</label>
                <textarea
                  value={loc.description}
                  onChange={e => updateLocation(loc.id!, { description: e.target.value })}
                  placeholder={t('list.placeholderDescription')}
              ...
              {/* 剧情重要性 */}
              <div>
                <label className="block text-xs text-text-muted mb-1">{t('list.labelSignificance')}</label>
                <textarea
                  value={loc.significance}
                  onChange={e => updateLocation(loc.id!, { significance: e.target.value })}
                  placeholder={t('list.placeholderSignificance')}
              ...
              <div className="flex items-center justify-between">
                ...
                {isConfirmingDelete ? (
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-red-400">{t('list.confirmDeletePrompt')}</span>
                    <button
    ...
    <div className="max-w-4xl">
      ...
      <div className="mb-4 flex items-center justify-between flex-wrap gap-2">
        <div className="text-sm text-text-secondary">
          {t('toolbar.count', { count: locations.length })}
        </div>
        <div className="flex items-center gap-2">
          {/* 视图切换 */}
          <div className="flex bg-bg-elevated rounded-lg p-0.5">
            ...
            >
              <GitBranch className="w-3.5 h-3.5" /> {t('toolbar.viewTree')}
            </button>
            <button
            ...
            >
              <List className="w-3.5 h-3.5" /> {t('toolbar.viewList')}
            </button>
          </div>
          <button
          ...
          >
            ...
            {extracting ? t('toolbar.extracting') : t('toolbar.extractAi')}
          </button>
        </div>
      </div>

      {extracting && progress && (
        <div className="mb-3 rounded-xl border border-accent/20 bg-accent/10 p-3">
          <div className="flex items-center gap-2 text-sm text-accent">
            <Loader2 className="h-4 w-4 animate-spin" />
            {t('toolbar.extracting')} · {progress.done}/{progress.total}
          </div>
      ...
      {(extracting || (extractError && !recoverable) || extractRunId != null || candidates.length > 0) && (
        <ExtractionReviewPanel
          title={t('extraction.candidatesTitle')}
          items={candidates}
      ...
      {view === 'tree' && (
        <div className="mb-6">
          ...
          />
          {locations.length > 0 && (
            <p className="text-xs text-text-muted mt-2 text-center">{t('list.treeJumpHint')}</p>
          )}
        </div>
      )}
        ...
        <div>
          {locations.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-text-muted">
              ...
              <p className="text-sm mb-3">{t('list.empty')}</p>
              <button
                onClick={() => handleAdd(null)}
