...
export default function LocationPanel({ project }: Props) {
  const { t, lang } = useDomainT('location')
  const {
  ...
  const handleExtractLocations = async () => {
    ...
    if (written.length === 0) {
      setExtractError(t('extraction.noWrittenChapters'))
      return
    }
    setExtracting(true)
    ...
    try {
      ...
      setExtractError(error instanceof Error ? error.message : t('extraction.failed'))
    } finally {
      setExtracting(false)
    }
  }
  ...
  const handleAdoptLocations = async () => {
    ...
    if (!result.written.length && result.skipped.length) {
      setExtractError(new Intl.ListFormat(lang, { type: 'conjunction', style: 'short' }).format(result.skipped.map(item => item.reason)))
      return
    }
