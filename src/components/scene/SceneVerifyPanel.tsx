...
export default function SceneVerifyPanel({ project }: Props) {
  ...
  return (
    <div className="max-w-3xl space-y-5">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-xl font-bold text-text-primary flex items-center gap-2">
              <ScanSearch className="w-5 h-5" /> {t('panel.title')}
            </h2>
      ...
      {/* 场景输入 */}
      <section>
        <label className="block text-sm font-medium text-text-primary mb-2">{t('form.sceneLabel')}</label>
        <AutoResizeTextarea
          value={scene}
          onChange={e => setScene(e.target.value)}
          placeholder={t('form.scenePlaceholder')}
      ...
      <section className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs text-text-muted mb-1">{t('form.eraLabel')}</label>
          <CInput
            value={sceneEra}
            onChange={e => setSceneEra(e.target.value)}
            placeholder={t('form.eraPlaceholder')}
        ...
        <div>
          <label className="block text-xs text-text-muted mb-1">{t('form.locationLabel')}</label>
          <CInput
            value={sceneLocation}
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          {building ? t('action.building') : ai.isStreaming ? t('action.streaming') : t('action.button')}
        </button>
        {ai.isStreaming && (
          <button onClick={ai.stop} className="text-xs text-text-muted hover:text-red-500 transition-colors">{t('action.stop')}</button>
        )}
      </div>
