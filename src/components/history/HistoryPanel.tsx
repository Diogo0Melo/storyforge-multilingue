...
export default function HistoryPanel({ project }: Props) {
  ...
  const handleDeleteKeyword = async (id: number) => {
    const ok = await dialog.confirm({
      title: t('keyword.deleteConfirmTitle'),
      message: t('common:deleteConfirm'),
      confirmText: t('common:delete'),
      tone: 'danger',
    })
  ...
  const handleAddEvent = async () => {
    ...
    const newId = await addEvent({
      ...
      era: 'custom',
      year: 0,
      date: t('timeline.defaultDate'),
      title: t('timeline.newEventTitle'),
      description: t('timeline.newEventDescription'),
      isHistorical: true,
  ...
  const handleAddKeyword = async () => {
    ...
    const newId = await addKeyword({
      projectId: project.id!,
      keyword: t('keyword.newKeyword'),
      category: 'technology',
      era: 'custom',
      description: t('keyword.newKeywordDescription'),
      ...(scopeGroupId != null ? { worldGroupId: scopeGroupId } : {}),
    })
      ...
      <header className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-semibold text-text-primary">{t('panel.title')}</h1>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-medium text-text-secondary">
                {t('timeline.sectionTitle', { count: scopedEvents.length })}
                {isMW && worldTab === 'all' && <span className="ml-1 text-text-muted">{t('panel.readOnlySuffix')}</span>}
              </h3>
              {canEdit && (
                <button
            ...
            {loadingEvents ? (
              ...
              <div className="rounded-2xl border border-dashed border-border bg-bg-elevated/10 p-12 text-center">
                ...
                <h4 className="text-sm font-medium text-text-primary mb-1">{t('timeline.emptyTitle')}</h4>
                <p className="text-xs text-text-muted mb-4">{t('timeline.emptyHint')}</p>
                <button
                  onClick={handleAddEvent}
              ...
              <div className="relative pl-6 border-l border-border/80 space-y-4 ml-3">
                {scopedEvents.map(event => {
                  ...
                  const worldBadge = isMW && worldTab === 'all' && event.worldGroupId != null
                    ? { icon: group?.icon || '🌐', name: group?.name || t('timeline.unknownWorld') }
                    : undefined
                  return (
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 bg-bg-surface border border-border rounded-xl p-3">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                >
                  <option value="all">{t('keyword.filterCategoryAll')}</option>
                  {(Object.keys(KEYWORD_CATEGORY_LABELS) as HistoricalKeywordCategory[]).map(k => (
                    <option key={k} value={k}>{t(`keywordCategories.${k}`)}</option>
                  ))}
                </select>
                <select
                  value={filterEra}
                  ...
                  className="px-2 py-1 bg-bg-base border border-border rounded-lg text-xs text-text-primary focus:outline-none focus:border-accent"
                >
                  <option value="all">{t('keyword.filterEraAll')}</option>
                  {(Object.keys(HISTORICAL_ERA_LABELS) as HistoricalEra[]).map(k => (
                    <option key={k} value={k}>{t(`eraLabels.${k}`)}</option>
                  ))}
                </select>
              </div>
            ...
            {loadingKeywords ? (
              ...
              <div className="rounded-2xl border border-dashed border-border bg-bg-elevated/10 p-12 text-center">
                ...
                <h4 className="text-sm font-medium text-text-primary mb-1">{t('keyword.emptyTitle')}</h4>
                <p className="text-xs text-text-muted mb-4">{t('keyword.emptyHint')}</p>
                {keywords.length === 0 && (
                  <button
                    onClick={handleAddKeyword}
                    className="inline-flex items-center gap-1.5 px-4 py-2 bg-accent text-white text-xs rounded-lg hover:opacity-90"
