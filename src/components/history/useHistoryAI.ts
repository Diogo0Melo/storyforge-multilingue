...
export function useHistoryAI(options: Options) {
  ...
  const setLane = useCallback((
    mode: HistoryAIMode,
    next: HistoryAgentLaneState | ((current: HistoryAgentLaneState) => HistoryAgentLaneState),
  ) => {
  ...
  const runRequest = useCallback(async (
    ...
    if (!force && (current.busy || current.candidate || current.unsafeRunId != null)) return
  ...
  const retry = useCallback(async (mode: HistoryAIMode) => {
    ...
    try {
      ...
      await runRequest(mode, request, true)
    } catch (error) {
      const message = messageOf(error)
      setLane(mode, { ...current, busy: false, message })
