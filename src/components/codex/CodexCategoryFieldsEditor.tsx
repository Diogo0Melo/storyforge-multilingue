...
export default function CodexCategoryFieldsEditor({ category, onClose, onSave }: Props) {
  ...
  // 内置字段按 labelKey/optionKeys 解析多语显示(只影响呈现);编辑时 onChange 清空键,手动文本优先。
  const displayLabel = (definition: CodexFieldDef) =>
    definition.labelKey ? t(definition.labelKey as any, definition.label) : definition.label
  const displayOptions = (definition: CodexFieldDef) =>
    (definition.options || []).map((option, i) =>
      definition.optionKeys?.[i] ? t(definition.optionKeys[i] as any, option) : option
    )
  // 内置分类标题按 builtInKey 解析多语名,回退持久化 name
  const categoryTitle = category.builtInKey
    ? t(`builtinCategories.${category.builtInKey}` as any, category.name)
    : category.name
  ...
  const add = () => setDefs(current => [...current, {
    ...
    label: t('fieldsEditor.newFieldDefaultLabel'),
    type: 'text',
  }])
      ...
      <div className="bg-bg-surface border border-border rounded-xl w-full max-w-lg max-h-[80vh] flex flex-col" onClick={event => event.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          <h3 className="text-sm font-bold text-text-primary flex items-center gap-2">
            <Settings2 className="w-4 h-4 text-accent" /> {t('fieldsEditor.title', { category: categoryTitle })}
          </h3>
          <button onClick={onClose} className="p-1 text-text-muted hover:text-text-primary" aria-label={t('fieldsEditor.closeAria')}>
            <X className="w-4 h-4" />
          </button>
        </div>

        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {defs.length === 0 && <p className="text-xs text-text-muted text-center py-4">{t('fieldsEditor.emptyHint')}</p>}
          {defs.map((definition, index) => (
            <div key={definition.key} className="border border-border rounded-lg p-2 space-y-1.5 bg-bg-base">
              <div className="flex items-center gap-1.5">
                ...
                >
                  <option value="text">{t('fieldsEditor.fieldTypeText')}</option>
                  <option value="longtext">{t('fieldsEditor.fieldTypeLongtext')}</option>
                  <option value="select">{t('fieldsEditor.fieldTypeSelect')}</option>
                  <option value="number">{t('fieldsEditor.fieldTypeNumber')}</option>
                  <option value="ref">{t('fieldsEditor.fieldTypeRef')}</option>
                </select>
                <button onClick={() => move(index, -1)} disabled={index === 0} className="p-1 text-text-muted hover:text-text-primary disabled:opacity-30" aria-label={t('fieldsEditor.moveUpAria', { label: displayLabel(definition) })}>
                  <ChevronUp className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => move(index, 1)} disabled={index === defs.length - 1} className="p-1 text-text-muted hover:text-text-primary disabled:opacity-30" aria-label={t('fieldsEditor.moveDownAria', { label: displayLabel(definition) })}>
                  <ChevronDown className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => remove(index)} className="p-1 text-text-muted hover:text-red-400" aria-label={t('fieldsEditor.removeAria', { label: displayLabel(definition) })}>
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>
              {definition.type === 'select' && (
                <input
                  value={displayOptions(definition).join(' / ')}
                  onChange={event => update(index, { options: event.target.value.split('/').map(item => item.trim()).filter(Boolean), optionKeys: undefined })}
                  placeholder={t('fieldsEditor.selectOptionsPlaceholder')}
              ...
              {definition.type === 'ref' && (
                <input
                  ...
                  placeholder={t('fieldsEditor.refCategoryPlaceholder')}
                  className="w-full px-2 py-1 text-xs rounded bg-bg-elevated border border-border focus:outline-none focus:border-accent"
                />
              )}
            </div>
        ...
        <div className="flex items-center justify-between px-3 py-2.5 border-t border-border">
          <button onClick={add} className="px-2.5 py-1.5 text-xs rounded-lg border border-dashed border-border text-text-muted hover:text-accent hover:border-accent/50 inline-flex items-center gap-1">
            <Plus className="w-3.5 h-3.5" /> {t('fieldsEditor.addField')}
          </button>
          <div className="flex items-center gap-2">
            <button onClick={onClose} className="px-3 py-1.5 text-xs text-text-secondary hover:text-text-primary">{t('common:cancel')}</button>
            <button
