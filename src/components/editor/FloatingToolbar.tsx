    ...
    >
      ...
      {busy && (
        <div className="min-w-[220px] rounded-lg border border-accent/30 bg-bg-elevated px-3 py-2 shadow-lg">
          <div className="flex items-center gap-2 text-xs text-accent"><Loader2 className="h-3 w-3 animate-spin" />{t('floatingToolbar.processing')}</div>
        </div>
      )}
          ...
          <div className="flex items-center gap-2">
            {candidate?.mode === 'edit' && (
              <button type="button" onClick={() => { void handleAccept() }}
                ...
                <Check className="h-3 w-3" />{t('floatingToolbar.btnReplace')}
              </button>
            )}
            {unsafeRunId != null ? (
              <button type="button" onClick={() => { void handleAbandon() }}
                className="rounded px-2 py-1 text-xs text-red-400 hover:bg-red-400/10">{t('common:cancel')}</button>
            ) : (
