    ...
    >
      ...
      <div className={`flex items-center gap-0.5 transition-opacity shrink-0 mt-1 ${
        ...
        {!ch.summary.trim() && onGenerate && (
          <button onClick={onGenerate} className="p-1 text-text-muted hover:text-accent rounded" title={t('tree.aiGenerateChapter')}>
            <Sparkles className="w-3.5 h-3.5" />
          </button>
        )}
        {onInsertAfter && (
          <button onClick={onInsertAfter} className="p-1 text-text-muted hover:text-accent rounded" title={t('tree.insertBelow')}>
            <CornerDownRight className="w-3.5 h-3.5" />
          </button>
        )}
        {onOpen && (
          <button onClick={() => onOpen(ch.id!)} className="p-1 text-text-muted hover:text-accent rounded" title={t('tree.editChapter')}>
...
export function OutlineStoryBlockSection({
  ...
  const handleDeleteBlock = async () => {
    ...
    const ok = await dialog.confirm({
      title: t('tree.deleteBlockTitle', { name: block.title }),
      message: t('tree.deleteBlockMessage'),
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden">
      ...
      >
        ...
        />
        <span className="text-[10px] text-text-muted">{t('tree.chaptersCount', { count: chapters.length })}</span>
        <button onClick={onAddChapter} className="p-1 text-text-muted hover:text-accent rounded" title={t('tree.addChapter')}>
