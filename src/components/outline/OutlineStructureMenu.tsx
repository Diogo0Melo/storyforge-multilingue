  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)}
        ...
        <LayoutList className="w-3 h-3" /> {t('structure.add')}
      </button>
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 z-20 bg-bg-elevated border border-border rounded-lg shadow-lg py-1 min-w-[160px]">
            {(Object.entries(STORY_STRUCTURES) as [StoryStructure, { label: string; labelKey: string; blocks: string[] }][]).map(([key, definition]) => (
              <button key={key} onClick={() => { onSelect(key); setOpen(false) }}
                ...
                <span className="font-medium">{t(definition.labelKey, { defaultValue: definition.label })}</span>
                {definition.blocks.length > 0 && (
                  <span className="text-text-muted ml-1">({t('structure.blockCount', { count: definition.blocks.length })})</span>
                )}
              </button>
            ))}
          </div>
