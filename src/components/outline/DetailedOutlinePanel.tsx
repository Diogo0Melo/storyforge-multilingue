...
export default function DetailedOutlinePanel({ project }: Props) {
  ...
  const addScene = async () => {
    ...
    const newScene: DetailedScene = {
      sceneId: nanoid(),
      title: t('detailed.newScene'), summary: '',
  ...
  const handleAcceptEnhanced = async (text: string) => {
    ...
    if (!parsed) {
      toast.error(t('detailed.parseFailed'))
      return
    }
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {!currentChapter ? (
          <div className="h-full flex items-center justify-center text-text-muted text-sm">
            {t('detailed.selectChapter')}
          </div>
        ) : (
          <>
            {/* 章节头 */}
            <div className="mb-4">
              <h2 className="text-xl font-bold text-text-primary mb-1">{t('detailed.panelTitle')}{currentChapter.title}</h2>
              <p className="text-sm text-text-muted">
                {currentChapter.summary || t('detailed.summaryEmpty')}
              </p>
            ...
            {isSyncStale && (
              <div className="mb-3 flex items-start gap-2 bg-warning/10 border border-warning/30 rounded-lg px-3 py-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-warning">{t('detailed.outlineChanged')}</p>
                  <p className="text-[11px] text-text-muted mt-0.5">
                    {t('detailed.outlineChangedDesc')}
                  </p>
                </div>
                <button
                  onClick={markSynced}
                  ...
                  onAccept={async (text) => {
                    try {
                      ...
                      if (newScenes.length === 0) {
                        toast.error(t('detailed.parseSceneFailed'))
                        return
                      }
                      ...
                      toast.success(t('detailed.adoptedScenes', { count: newScenes.length }))
                    } catch (err) {
                      console.error('[DetailedOutline] 采纳失败:', err)
                      toast.error(t('detailed.adoptFailed'))
                    }
                    ai.reset()
                  }}
            ...
            {currentDetailed && (
              ...
              <div className="mb-4 bg-bg-surface border border-border rounded-xl p-3 space-y-2">
                <h3 className="text-xs font-semibold text-text-muted uppercase tracking-wide">{t('detailed.enhancedInfo')}</h3>
                {currentDetailed.openingHook && (
                  <div>
                ...
                <div className="flex items-center gap-4 text-xs">
                  ...
                  {currentDetailed.appearingCharacterIds && currentDetailed.appearingCharacterIds.length > 0 && (
                    <span className="text-text-secondary">
                      👥 {t('detailed.characters', { count: currentDetailed.appearingCharacterIds.length })}
                    </span>
                  )}
                  {currentDetailed.foreshadowIds && currentDetailed.foreshadowIds.length > 0 && (
                    <span className="text-text-secondary">
                      🔮 {t('detailed.foreshadows', { count: currentDetailed.foreshadowIds.length })}
                    </span>
                  )}
                </div>
                {currentDetailed.prohibitions && currentDetailed.prohibitions.length > 0 && (
                  <div className="border-t border-border pt-2">
                    <span className="text-[10px] text-warning">{t('detailed.prohibitions')}</span>
