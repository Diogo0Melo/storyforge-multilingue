  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-2 space-y-1.5">
        ...
        >
          <Plus className="w-3.5 h-3.5" /> {t('volume.add')}
        </button>
        <button
        ...
        >
          <Sparkles className="w-3.5 h-3.5" /> {t('volume.batchGenerate')}
        </button>
        {volumes.length >= 2 && (
          <button
          ...
          >
            <Layers className="w-3.5 h-3.5" /> {t('volume.batchGenerateAllChapters')}
          </button>
        )}
      </div>
            ...
            {batchRunning && batchProgress && (
              <>
                <div className="flex items-center gap-1.5 text-xs text-accent">
                  ...
                  <span>{t('batch.progress', { completed: batchProgress.completedVolumes, total: batchProgress.totalVolumes })}</span>
                </div>
            ...
            {!batchRunning && batchResult && (
              <>
                <p className="text-xs text-success">
                  {t('batch.complete', { count: Array.from(batchResult.values()).reduce((sum, chapters) => sum + chapters.length, 0) })}
                </p>
                <div className="flex gap-1">
                  ...
                  >
                    <Check className="w-3 h-3" /> {t('batch.writeAll')}
                  </button>
                  <button
                    onClick={onDismissBatch}
                    title={t('batch.dismissTitle')}
      ...
      <div className="flex-1 overflow-y-auto px-1">
            ...
            >
              ...
              >
                ...
                <p className="text-[10px] text-text-muted">
                  {t('volume.chapterCount', { count: childCount })}{volume.summary ? ` · ${volume.summary.slice(0, 20)}...` : ''}
                </p>
              </button>
            </div>
          )
        ...
        {volumes.length === 0 && (
          <div className="text-center py-8 text-text-muted text-xs">{t('volume.emptyState')}</div>
        )}
      </div>
    </div>
  )
