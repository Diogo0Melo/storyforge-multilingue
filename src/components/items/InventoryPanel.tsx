...
export default function InventoryPanel({ project }: Props) {
  ...
  const handleManualAdd = async () => {
    if (!selectedCharacter) {
      setExtractError(t('items.inventory.selectCharPlease' as any))
      return
    }
    await addEntry({
      projectId: project.id!,
      itemName: t('items.inventory.newItem' as any),
      heldByName: selectedCharacter.name,
      characterId: selectedCharacter.id ?? null,
      action: 'gain',
      quantity: 1,
      note: t('items.inventory.manualNote' as any),
    })
  }
    ...
    <div className="max-w-3xl space-y-5">
      ...
      <div className="pb-4 border-b border-border/40">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-xl font-bold text-text-primary flex items-center gap-2">
              <Package className="w-5 h-5" /> {t('items.inventory.title' as any)}
            </h2>
            <p className="text-xs text-text-muted mt-0.5">
              {t('items.inventory.subtitle' as any)}
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              title={selectedCharacter ? t('items.inventory.addFor' as any, { name: selectedCharacter.name }) : t('items.inventory.selectCharFirst' as any)}
            ...
            >
              <Plus className="w-3.5 h-3.5" /> {t('items.inventory.manualAdd' as any)}
            </button>
            <button
            ...
            >
              ...
              {extracting ? t('items.inventory.extracting' as any, { done: progress?.done ?? 0, total: progress?.total ?? 0 } as any) : t('items.inventory.extractFromText' as any)}
            </button>
          </div>
        </div>
        ...
        {characters.length > 0 && (
          <div className="mt-3 flex items-center gap-2">
            <span className="text-xs text-text-muted shrink-0">{t('items.inventory.viewCharacter' as any)}</span>
            <select
            ...
            >
              <option value="">{t('items.inventory.allCharacters' as any)}</option>
              {groupedCharacters.map(group => (
                <optgroup key={group.weight} label={t(`items.roleWeight.${group.weight}` as any)}>
                  {group.chars.map(c => (
                    <option key={c.id} value={c.id}>{c.name}{c.roleWeight === 'main' ? t('items.inventory.mainRole' as any) : ''}</option>
                  ))}
                </optgroup>
              ))}
            </select>
            {selectedCharacter && (
              <span className="text-xs text-accent">{t('items.inventory.currentBag' as any, { name: selectedCharacter.name } as any)}</span>
            )}
          </div>
        )}
        ...
        {extractionChapters.length > 0 && (
          <div className="mt-2 flex items-center gap-2 text-xs">
            <span className="text-text-muted">{t('items.inventory.extractRange' as any)}</span>
            <select
              value={extractMode}
            ...
            >
              <option value="all">{t('items.inventory.allChapters' as any)}</option>
              <option value="range">{t('items.inventory.customRange' as any)}</option>
            </select>
            {extractMode === 'range' && (
              <>
                <select
                ...
                >
                  {extractionChapters.map(item => (
                    <option key={item.chapter.id ?? item.ordinal} value={item.ordinal}>
                      {t('items.inventory.chapter' as any, { ordinal: item.ordinal, title: item.chapter.title } as any)}{item.hasWrittenContent ? '' : t('items.inventory.noContent' as any)}
                    </option>
                  ))}
                </select>
                <span className="text-text-muted">{t('items.inventory.to' as any)}</span>
                <select
                  value={extractEnd}
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-xl border border-border bg-bg-surface p-3">
          <p className="text-[10px] uppercase tracking-wide text-text-muted">{t('items.inventory.activeKinds' as any)}</p>
          <p className="text-xl font-semibold text-text-primary mt-1">{inventoryStats.activeKinds}</p>
        </div>
        <div className="rounded-xl border border-border bg-bg-surface p-3">
          <p className="text-[10px] uppercase tracking-wide text-text-muted">{t('items.inventory.totalHeld' as any)}</p>
          <p className="text-xl font-semibold text-green-400 mt-1">{inventoryStats.totalHeld}</p>
        </div>
        <div className="rounded-xl border border-border bg-bg-surface p-3">
          <p className="text-[10px] uppercase tracking-wide text-text-muted">{t('items.inventory.movements' as any)}</p>
      ...
      {extracting && progress && (
        <div className="p-3 bg-accent/10 border border-accent/20 rounded-lg">
          <div className="flex items-center gap-2 text-sm text-accent mb-1.5">
            ...
            {t('items.inventory.extractingProgress' as any, { done: progress.done, total: progress.total } as any)}
          </div>
      ...
      {/* 物品栏 */}
      {loading ? (
        <div className="text-text-muted text-sm py-8 text-center">{t('items.inventory.loading' as any)}</div>
      ) : inventory.length === 0 ? (
        <div className="text-center py-12 text-text-muted">
          <Package className="w-10 h-10 mx-auto mb-3 opacity-30" />
          <p className="text-sm">{t('items.inventory.empty' as any)}</p>
          <p className="text-xs mt-1">{t('items.inventory.emptyHint' as any)}</p>
        </div>
      ) : (
              ...
              <div key={inventoryKey} className={`bg-bg-surface border rounded-xl overflow-hidden ${
                ...
                >
                  ...
                  <span className="text-[10px] text-text-muted flex-1">
                    {t('items.inventory.cumulativeGain' as any, { count: gained, count2: consumed } as any)}
                  </span>
                  ...
                  <span className={`text-[10px] px-1.5 py-0.5 rounded shrink-0 ${
                    ...
                    {item.quantity > 0 ? t('items.inventory.holding' as any) : item.quantity === 0 ? t('items.inventory.depleted' as any) : t('items.inventory.needsCheck' as any)}
                  </span>
                </button>
                ...
                {isOpen && (
                  <div className="border-t border-border/50 p-3">
                    <p className="text-[10px] uppercase tracking-wide text-text-muted mb-2">{t('items.inventory.gainConsumeTimeline' as any)}</p>
                      ...
                      <div key={e.id} className="relative flex flex-wrap items-center gap-2 pl-4 py-2 text-xs group">
                        ...
                        <div className="ml-auto flex flex-wrap items-center justify-end gap-1.5 shrink-0">
                          <input
                            ...
                            title={t('items.inventory.editItemName' as any)}
                            aria-label={t('items.inventory.editItemName' as any)}
                          ...
                          <input
                            ...
                            placeholder={t('items.inventory.holderPlaceholder' as any)}
                            title={t('items.inventory.editHolder' as any)}
                            aria-label={t('items.inventory.editHolder' as any)}
                          ...
                          <input
                            ...
                            title={t('items.inventory.editQuantity' as any)}
                            aria-label={t('items.inventory.editQuantity' as any)}
                          ...
                          >
                            <option value="gain">{t('items.inventory.gain' as any)}</option>
                            <option value="consume">{t('items.inventory.consume' as any)}</option>
                          </select>
                          <button
                            onClick={() => deleteEntry(e.id!)}
                            title={t('items.inventory.deleteEntry' as any)}
                            aria-label={t('items.inventory.deleteEntry' as any)}
                          ...
                          <input
                            ...
                            placeholder={t('items.inventory.notePlaceholder' as any)}
                            title={t('items.inventory.editNote' as any)}
                            aria-label={t('items.inventory.editNote' as any)}
      ...
      {unclaimedEntries.length > 0 && selectedCharacterId == null && (
        <div className="border border-dashed border-border/60 rounded-xl p-4 bg-bg-elevated/30">
          <p className="text-xs text-text-muted mb-2">
            {t('items.inventory.unclaimed' as any)}
          </p>
          <div className="text-[10px] text-text-muted space-y-1">
            {unclaimedEntries.map(e => (
              <div key={e.id} className="flex items-center gap-2">
                <span>{e.itemName}</span>
                <span className="text-text-muted/60">{e.action === 'gain' ? t('items.inventory.gain' as any) : t('items.inventory.consume' as any)} ×{e.quantity}</span>
                {e.chapterTitle && <span className="text-text-muted/60">· {e.chapterTitle}</span>}
                <select
                  value=""
                  aria-label={t('items.inventory.claimAria' as any, { name: e.itemName } as any)}
                  onChange={event => {
                ...
                >
                  <option value="">{t('items.inventory.selectCharToClaim' as any)}</option>
