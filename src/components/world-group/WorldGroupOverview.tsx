...
export default function WorldGroupOverview({ project }: Props) {
  const { t, lang } = useDomainT('world-group')
  ...
  const handleAISuggest = async () => {
    ...
    const messages = buildWorldSuggestPrompt({
      ...
      genres: new Intl.ListFormat(lang, { type: 'conjunction', style: 'short' }).format(project.genres || []),
      concept: concept || project.description || t('overview.suggestFallbackConcept'),
      existingWorlds,
      userHint: '',
    })
  ...
  const handleAddWorld = async () => {
    const id = await createGroup({
      projectId: project.id!,
      name: t('overview.newWorldDefaultName'),
      description: '',
    ...
    const created = groups.find(g => g.id === id) ||
      { id, projectId: project.id!, name: t('overview.newWorldDefaultName'), description: '', type: 'traversal' as WorldGroupType, icon: '🌐', order: groups.length, createdAt: Date.now(), updatedAt: Date.now() }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {loading ? (
        <div className="text-text-muted text-sm py-8 text-center">{t('overview.loading')}</div>
      ) : (
        <>
          {/* 世界列表 */}
          <section className="space-y-2">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-text-primary">{t('overview.listTitle', { count: groups.length })}</h3>
            ...
            {showSuggest && (
              <div className="p-3 bg-bg-surface border border-border rounded-lg space-y-2.5">
                <textarea
                  ...
                  placeholder={t('overview.suggestPlaceholder')}
                  rows={2}
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    {ai.isStreaming ? t('overview.aiThinking') : t('overview.generateSuggestions')}
                  </button>
                  {ai.isStreaming && (
                    <button onClick={ai.stop} className="text-xs text-text-muted hover:text-red-500">{t('overview.stopGeneration')}</button>
                  )}
                </div>
                  ...
                  <div className="space-y-1.5 pt-1">
                    {suggested.map((w, i) => (
                      <div key={i} className="flex items-start gap-2 p-2.5 bg-bg-base border border-border rounded-lg">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5">
                            ...
                            {w.plannedChapterCount > 0 && (
                              <span className="text-[10px] text-text-muted">{t('overview.chaptersSuffix', { count: w.plannedChapterCount })}</span>
                            )}
                          </div>
                          <p className="text-xs text-text-muted mt-0.5">{w.description}</p>
                          {w.entryCondition && <p className="text-[10px] text-text-muted mt-0.5">{t('overview.entryPrefix')}{w.entryCondition}</p>}
                        </div>
                        <button
                        ...
                        >
                          {adoptedIdx.has(i) ? <><Check className="w-3 h-3" /> {t('overview.adopted')}</> : t('overview.adopt')}
                        </button>
                      </div>
                    ))}
                  </div>
                ...
                {suggested && suggested.length === 0 && (
                  <p className="text-xs text-text-muted">{t('overview.noValidSuggestions')}</p>
                )}
              </div>
            )}
              ...
              {groups.map(g => (
                ...
                >
                  ...
                  {g.plannedChapterCount ? (
                    <span className="text-xs text-text-muted shrink-0">{t('overview.chaptersSuffix', { count: g.plannedChapterCount })}</span>
                  ) : null}
                    ...
                    {g.type !== 'primary' && (
                      confirmDeleteId === g.id ? (
                        ...
                        <button
                          ...
                          title={t('overview.deleteWorldTitle')}
                        >
          ...
          {groups.length > 1 && (
            <section className="space-y-2">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-text-primary">{t('overview.relationsTitle')}</h3>
                <button
                  onClick={() => setShowLinkForm(v => !v)}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg bg-bg-elevated text-text-secondary border border-border hover:text-accent hover:border-accent/50 transition-colors"
                >
                  <Plus className="w-3.5 h-3.5" />
                  {t('overview.addRelationButton')}
                </button>
              </div>
              ...
              {showLinkForm && (
                <div className="flex items-center gap-2 flex-wrap p-3 bg-bg-surface border border-border rounded-lg">
                  ...
                  >
                    <option value="">{t('overview.fromWorldPlaceholder')}</option>
                  ...
                  <select
                    ...
                    className="px-2 py-1.5 bg-bg-base border border-border rounded text-xs text-text-primary focus:outline-none focus:border-accent"
                  >
                    <option value="">{t('overview.toWorldPlaceholder')}</option>
                    {groups.map(g => <option key={g.id} value={g.id}>{g.icon} {g.name}</option>)}
                  </select>
                  <select
                    value={linkForm.type}
                    ...
                    className="px-2 py-1.5 bg-bg-base border border-border rounded text-xs text-text-primary focus:outline-none focus:border-accent"
                  >
                    {(['portal', 'ascension', 'summon', 'branch', 'return', 'custom'] as WorldGroupLinkType[]).map(v => (
                      <option key={v} value={v}>{t(LINK_TYPE_KEY[v])}</option>
                    ))}
                  </select>
                  <input
                    value={linkForm.name}
                    onChange={e => setLinkForm(f => ({ ...f, name: e.target.value }))}
                    placeholder={t('overview.channelNamePlaceholder')}
                  ...
                  {links.map(l => {
                    ...
                    return (
                      <div key={l.id} className="flex items-center gap-2 px-3 py-2 bg-bg-surface border border-border rounded-lg text-sm group">
                        ...
                        <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-bg-elevated text-text-muted border border-border/50">
                          {t(LINK_TYPE_KEY[l.linkType])}
                        </span>
                        {l.name && <span className="text-text-muted text-xs">{t('overview.linkNameSuffix', { name: l.name })}</span>}
                        <button
                          onClick={() => deleteLink(l.id!)}
                          className="ml-auto p-1 rounded text-text-muted hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"
                          title={t('overview.deleteRelationTitle')}
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      </div>
          ...
          {groups.filter(g => g.type !== 'primary').length > 0 && (
            <section className="space-y-2">
              <h3 className="text-sm font-semibold text-text-primary">{t('overview.traversalOverviewTitle')}</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left py-2 px-3 text-xs font-medium text-text-muted">{t('overview.colWorld')}</th>
                      <th className="text-left py-2 px-3 text-xs font-medium text-text-muted">{t('overview.colType')}</th>
                      <th className="text-left py-2 px-3 text-xs font-medium text-text-muted">{t('overview.colPlannedChapters')}</th>
                      <th className="text-left py-2 px-3 text-xs font-medium text-text-muted">{t('overview.colEntryCondition')}</th>
                      <th className="text-left py-2 px-3 text-xs font-medium text-text-muted">{t('overview.colPowerRestriction')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {groups.filter(g => g.type !== 'primary').map(g => (
                      <tr key={g.id} className="border-b border-border/50 hover:bg-bg-hover/50 transition-colors">
                        ...
                        <td className="py-2 px-3 text-text-muted text-xs">{t(TYPE_KEY[g.type])}</td>
                        <td className="py-2 px-3 text-text-muted">{g.plannedChapterCount ? t('overview.chaptersSuffix', { count: g.plannedChapterCount }) : '—'}</td>
