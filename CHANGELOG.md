...
- 新增与分步骤模式平级的独立节点模式：支持六类节点、动态输入槽、自由连线、分支合流、局部执行、逐节点上下文裁剪记录、运行恢复和输出节点显式采纳。
- 新增角色物品所有权、角色知识账本、世界宪法、角色生命周期和故事线进度等长期一致性能力，并把迁移、删除、合并、导入导出纳入统一注册表生命周期。
- 完成透明生成工坊、角色驱动方案、安全角色改稿、实体改名和作者风格校准，AI 结果均经过预览、质量闸门和作者确认。
- 完成世界知识归属、修炼体系与进度、导入词条分类及世界地图空间约束；完成灵感增量融合和参考分析版本演化。
- 精简 Agent 固定项目上下文并建立按任务路由，减少无关文档重复输入；新增统一创作平台与本地数据/RAG 分层设计。

## v3.8.1 — 2026-07-26 · 架构治理与发布基线

- 完成 GOV-1 第一阶段架构治理：AI Manual 改用 AST 扫描并增加独立真实性测试；参考分析写回统一进入字段/采纳注册表；架构守卫覆盖 `src/lib` 旁路和旧 context builder；事实账本成为有复审日期的正式领域扩展。
- 生产依赖漏洞清零并接入 CI；运行时代码覆盖率排除静态 Prompt 数据并按 AI/import/registry 分层设门槛；Blueprint 实时规模自动生成，界面版本增加构建 commit 标识，并新增安全报告政策。
