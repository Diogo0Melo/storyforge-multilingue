...
async function seed(): Promise<{
  ...
  const projectId = await db.projects.add({
    ...
    targetWordCount: 100_000, worldCode: `replan-${now}`, worldVersion: 1, createdAt: now, updatedAt: now,
  } as any) as number
...
describe.sequential('R-HARNESS57 · 人工修正后的 stale/replan', () => {
  ...
  it('工作区终验后立即 replan，来源编辑器恢复并展示三类计数', () => {
    ...
    expect(workspace).toContain("t('workspace.impactHandoff.statusCompleted')")
    expect(editor).toContain('readCurrentImpactPostCorrectionReplanV1')
    expect(editor).toContain("key: 'chapterEditor.impactRecoveredPostCorrectionPlan'")
    expect(editor).toContain('resolved: postCorrectionState.output.resolvedItemIds.length')
    expect(editor).toContain('remaining: postCorrectionState.output.remainingItemIds.length')
    expect(editor).toContain('new: postCorrectionState.output.newItemIds.length')
  })
})
