...
describe('AUDIT-6 · 章节记忆与计划对账视图', () => {
  it('无摘要时展示生成入口，并准确转发生成命令与忙碌态', async () => {
    ...
    expect(ready.host.textContent).toContain('memory.emptyHint')
    await act(async () => button(ready.host, 'memory.generateMemory').click())
    expect(ready.props.onGenerateMemory).toHaveBeenCalledOnce()

    const busy = await mount({ memoryBusy: true })
    expect(button(busy.host, 'memory.generating').disabled).toBe(true)
  })
  ...
  it('摘要可刷新，失效对账只显示刷新提示', async () => {
    ...
    expect(stale.host.textContent).toContain('memory.refreshMemory')
    expect(stale.host.textContent).toContain('memory.reconciliationStale')
    expect(stale.host.textContent).not.toContain('chapterMemory.reconciliationTitle')
  })

  it('当前待确认对账展示分类、证据并转发两种处理命令', async () => {
    const current = await mount({ reconciliation, reconciliationCurrent: true })
    expect(current.host.textContent).toContain('chapterMemory.reconciliationTitle')
    expect(current.host.textContent).toContain('chapterMemory.completedGoals')
    expect(current.host.textContent).toContain('chapterMemory.evidence')
    expect(current.host.textContent).toContain('chapterMemory.unfinishedGoals')
    await act(async () => button(current.host, 'chapterMemory.confirmProgress').click())
    await act(async () => button(current.host, 'chapterMemory.applyOutlineCandidate').click())
