...
describe('R-HARNESS59 · 分步骤 UI 模型入口注册表', () => {
  it('AST 守卫证明所有直调入口均登记且调用计数一致', () => {
    ...
    expect(output).toContain('governed 6, auxiliary 7, migration 0')
  })
  ...
  it('高风险正式写入入口不能冒充已治理或只读辅助', () => {
    ...
    expect(byFile.has('src/components/project/AnalysisReportViewer.tsx')).toBe(false)
  })

  it('已治理运行时与真正只读入口保留明确机制，迁移时可逐项清零', () => {
    expect(registry.entries.filter(entry => entry.status === 'governed')).toHaveLength(6)
    expect(registry.entries.filter(entry => entry.status === 'auxiliary')).toHaveLength(7)
    expect(registry.entries.filter(entry => entry.status === 'migration')).toHaveLength(0)
    expect(registry.entries.find(entry => entry.file.endsWith('useIncrementalInspiration.ts'))).toMatchObject({
      status: 'auxiliary',
      mechanism: 'preview-only',
    })
