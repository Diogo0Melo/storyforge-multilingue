...
describe('CF-20260703-4/5 · 角色关系保存反馈与角色词条同步', () => {
  ...
  it('只在采纳 AI 抽取关系后同步角色词条，手动占位关系不会污染人物关系描述', () => {
    ...
    expect(panelSource).not.toContain("toast.success(t('characterRelation.importSuccess')")
    expect(panelSource).toContain("toast.success(t('characterRelation.relationSaved'))")
  })
