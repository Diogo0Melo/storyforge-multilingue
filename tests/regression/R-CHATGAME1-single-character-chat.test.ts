...
describe('CHATGAME-1 · 单角色聊天 MVP', () => {
  ...
  it('历史消息、回复与重生成仍可回放，但不再开放新写入', async () => {
    ...
    await expect(configureChatSession({
      sessionId: session.id!, characterKey: 'character:keeper',
      identity: { name: '新身份', description: '' }, scene: { title: '新场景', description: '' },
    })).rejects.toThrow('只读兼容')
