...
import { classifyMasterCandidate } from '../../src/components/agent/useMasterCopilot'
import {
  appendAgentEvent,
...
import type { AgentEvent, WorkspaceScope } from '../../src/lib/types'
...
describe.sequential('R-HARNESS1-master-durable-orchestrator · 主 Agent durable 编排', { timeout: 15_000 }, () => {
  ...
  it('作者快速连续编辑 durable 候选时按顺序持久化，刷新恢复使用最新 hash', async () => {
    ...
    const earlierDraft = '较早的作者修订稿。'
    const revisedDraft = '作者确认后的最新潮汐世界来源。'
    ...
    expect((await readAgentRunV1(fixture.scope, result.runId)).events
      .filter(event => event.type === 'candidate.revised')).toHaveLength(2)
  ...
  it('损坏或不一致的 durable 候选被隔离，不会获得 legacy 路由', () => {
    const event: AgentEvent = {
      ...
      payload: JSON.stringify({
        ...
        baseSnapshot: {},
        runId: 9,
        runStepId: 'master:world-1',
      }),
      createdAt: Date.now(),
    }

    expect(classifyMasterCandidate(event)).toMatchObject({ kind: 'quarantined' })
  })

  it('legacy 编排器携带 dependencyBindings 时仍可编辑并沿 legacy 流程采纳', async () => {
    const fixture = await createWorkspace('legacy候选')
    const conversation = await getOrCreateAgentConversation({
      projectId: fixture.scope.projectId,
      worldGroupId: fixture.worldGroupId,
      scope: fixture.scope,
    ...
    expect(classifyMasterCandidate(event)).toMatchObject({ kind: 'legacy' })
    ...
    const edited = (await readAgentEvents(conversation.id!, fixture.scope))
      .find(row => row.id === event.id)!
    const classification = classifyMasterCandidate(edited)
    expect(classification).toMatchObject({ kind: 'legacy' })
    if (classification.kind !== 'legacy') throw new Error('legacy candidate unexpectedly quarantined')
    ...
    expect(await db.worldviews.toArray()).toEqual([
      expect.objectContaining({ worldOrigin: '作者编辑后的旧世界来源' }),
    ])
  })

  it('legacy creative artifact 的 malformed issues/status 会安全隔离且不能采纳', () => {
    const payload = {
      version: 1,
      taskId: 'world-1',
      agentId: 'world-origin',
    ...
    expect(() => classifyMasterCandidate(event)).not.toThrow()
    expect(classifyMasterCandidate(event)).toMatchObject({ kind: 'quarantined' })
    expect(() => assertMasterCreativeArtifactAdoptableV1(payload as any))
      .toThrow('创作产物合同无效')
  })
